...
#define DISABLE_USAGE	0					// Disable use of 'usage.sl3' as it currently (27-12-14) causes problems
#define USING_MTPOST	1
...
static int legacyReady = 0;							// 1 when legacy child is ready to accept calls
...
static int IsPubsubProtocol(nProtocol)
{
	...
	while ((szLine=ReadLine(fp))) {
		...
		if (chp-szLine+1 == nLen && !strnicmp(szName, szLine, chp-szLine+1)) {
			...
			if (szCloseQuote) {								// Fully quoted
				*szCloseQuote='\0';
				result = rcache_Decode(szValue+1);
			} else {										// Not quoted
...
// Searches for WAMPPREFIX in the binary and acts on what it finds
{
	...
	if (fp) {
		...
		while ((got=fread(buf+offset, 1, sizeof(buf)-offset-1, fp))) {
			...
			const char *pos;
			offset = 0;
			while ((pos = memmem(buf+offset, got-offset, WAMPPREFIX, len))) {	// pos = location within buffer of our search string
				...
				const char *eoString = memchr(buf, 0, got);
...
// If 'szFmt' starts "MESSAGE: " then only sends to the message log
{
	...
	va_list copy_ap;
	va_copy(copy_ap, ap);								// Need a copy if we're going to copy the stuff into the message log
...
//{FILE *fp=fopen("/tmp/fred","w");fprintf(fp, "Log file is '%s'\n", szLogFile);fclose(fp);}
	...
	if (bForMessage) {												// Wants to go into message log
		vsnprintf(buf, sizeof(buf), szMessage, copy_ap);
		va_end(copy_ap);
		NoteLog(buf);
...
static const char *HttpResponseText(int code)
{
	const char *s;
...
static int LegacyApiGone(CHAN *chan)
{
//Log("Child has gone - %s", chan->name);
	invoke_info *ii = chan_Info(chan);
	waitpid(ii->childPid, NULL, WNOHANG);				// Here need to see if the child crashed and/or burned

	JSON *yield = json_NewArray();

	HLIST *inq = chan_IncomingQueue(chan);
	int len = hlist_Length(inq);
	char *text = malloc(len);
	hlist_GetDataToBuffer(inq, len, text);
...
Log("OnInvoked(%p, %Ld, %s(%s), (%p)%s, (%p)%s)", wamp, requestId, procedure, binary, list, json_Render(list), dict, json_Render(dict));
	if (!binary) {
...
Log("Sending EOF to the API");
	...
	chan_RegisterClosed(fromChildChan, LegacyApiGone);
...
static void BecomeLegacyHandler(int fd)
// Become a WAMP handler for all legacy calls
// We talk through fd.
// We never return.
{
Log("BecomeLegacyHandler(%d)", fd);
	legacyPool = chan_PoolNew();									// Create a pool of connections for polling
	CHAN *channel = chan_NewFd(legacyPool, fd, CHAN_IN | CHAN_OUT);	// Create a connection for comms with parent
...
//Log("We have tried to call '%s'", procedure);
	...
	if (!strncmp(procedure, "mtxml.", 4)) {
		...
		if ((legacyChild = spiderFork("Legacy"))) {				// Parent
			...
			CHAN *channel = chan_NewFd(pool, legacyfds[0], CHAN_IN | CHAN_OUT);
			WS *ws = ws_NewOnChannel(channel);					// Create a new websocket connected to the channel
			WAMP *wamp = wamp_NewOnWebsocket(ws, 0);			// Add a wamp client with the websocket
Log("Parent to handle legacy on %d (channel = %s)", legacyfds[0], chan_Name(channel));
...
static int agent_OnClientChannelError(CHAN *channel)
// There's an error on the incoming client channel so we will delete all channels and thus drop out of the event loop.
{
Log("agent_OnClientChannelError(%s)", chan_Name(channel));
...
{
	Log("OnWampSpider(%s, %Ld, %s, %p, %p)", wamp_Name(wamp), requestId, procedure, argList, argDict);

	if (!strcasecmp(procedure, "spider.dump.json")) {
		json_LogAll();
		JSON *result = json_NewObject();
		json_ObjectAdd(result, "OK", json_NewBool(1));
...
static int OnPubsubConnected(WAMP *wamp, const char *mesg)
{
	if (wamp) {
		g_PubsubWamp = wamp;
...
Log("WARNING: Failed to make a connection to the pubsub server (%s) so can not act as a broker", mesg);
	...
	chan_RegisterError(c, agent_OnClientChannelError);
...
#if USING_MTPOST
#define WAMP_MTPOST		9999								// Our MTPOST message type

static int mtpost_Incoming(WAMP *wamp, int type, JSON *json)
{
	Log("MTPOST: On %s(%d) = %s", wamp_Name(wamp), type, json_Render(json));

	return 0;
}

static int mtpost_OnConnected(WAMP *wamp, const char *mesg)
{
Log("mtpost_OnConnected(%s)", wamp_Name(wamp));
	if (wamp) {
		g_PubsubWamp = wamp;

Log("Connected to the my neighbour on %s", wamp_Name(wamp));
...
Log("WARNING: Failed to make a connection to my neighbour: %s", mesg);
	}
...
// Old style RPC handling with the name attribute
{
	...
	if( (pid = spiderFork("OLD API HANDLER")) == 0 ) {
		/* Child Process */
...
static int OnWampRedirectConnected(WAMP *wamp, const char *mesg)
...
Log("Connected to the WAMP server (wampRedirectHost) on %s", wamp_Name(wamp));
	...
	json_ArrayAddStringz(hello, wampRedirectRealm);
	JSON *opts = json_NewObject();
	JSON *role = json_NewObject();
	json_ObjectAdd(role, "caller", json_NewObject());
	json_ObjectAdd(opts, "roles", role);
...
static time_t timeFromSSLTime(const char* sslTime)
...
{
	Log("Attempted PUBSUBS connection - can't do that yet");
}
...
// May actually be pubsub server
{
	...
	Log("SPIDER " VERSION " is entering %s mode", bIsPubsub ? "pubsub" : "daemon");
	...
	if (bIsDaemon) chan_PoolRegisterIdler(daemon_channel_pool, 1000, Idler);
...
//	json_Logger(vLog);
	...
	const char *szPortsPubsub = config_GetSetString("port-pubsub",bDefaultBase ? "4512" : NULL, "PUBSUB protocol ports (4512)");
