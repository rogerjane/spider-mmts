...
// 20-07-14 RJ 2.04 Enabled environments
...
int SetBaseDir(const char *szNewBaseDir)
// Given that the current directory is the base directory, get configuration.
// Returns	0	Failed, which means the base directory can't be valid
//			1	Everything seems to go well
{
	szDelete(szBaseDir);
...
int SetEnvironment(const char *szEnv)
...
int main(int argc, char *argv[])
{
	...
	while((c=getopt(argc,argv,"b:d:e:p:P:sSh:v"))!=-1){
		switch(c){
