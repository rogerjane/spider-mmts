...
// TODO: Keep up to date a table of usage statistics
...
static const char *szMsgLogDirLeaf = NULL;		// Last part of szMsgLogDir
...
// Plain errors
{
	...
	if (inFatal) exit(0);							// Recursive call to Fatal is a reason to simply leave.
	inFatal=1;
...
const char *cbac_GetId(const char *organisation, const char *product)
{
	...
	s3_MustQueryf(dbAccess, "SELECT id FROM productId WHERE organisation = %s AND product = %s", organisation, product);
...
// NoteRpcMessage(g_szRpcError, -1, "stderr", "txt", "Stdout returned from RPC handler");
{
	if (!_bNoteInhibit && g_szRpcError) {
		...
		if (fp) {
			...
			FileDescription("stderr.txt", "Debug output from API");
...
// Writes the management information to a file
{
	...
	char period[20];
	snprintf(period, sizeof(period), "hour-%02d", tm->tm_hour);
	S3 *s3Usage = UsageDatabase();
	S3LONG(s3Usage, count);

	s3_Queryf(s3Usage, "SELECT COUNT(*) count FROM usage WHERE period=%s AND api=%s AND caller=%s",
			period, mi_function, mi_caller);
	if (count) {
		s3_MustDof(s3Usage, "UPDATE usage SET calls=calls+1, duration=duration+%d WHERE period=%s AND api=%s AND caller=%s",
				mi_msecs, period,mi_function,mi_caller);
	} else {
		s3_MustDof(s3Usage, "INSERT INTO usage (calls,duration,period,api,caller) VALUES (1,%d,%s,%s,%s)",
				mi_msecs, period,mi_function,mi_caller);
	}

	s3_Queryf(s3Usage, "SELECT COUNT(*) count FROM usage WHERE period=%s AND api=%s AND caller IS NULL",
			period, mi_function);
	if (count) {
		s3_MustDof(s3Usage, "UPDATE usage SET calls=calls+1, duration=duration+%d WHERE period=%s AND api=%s AND caller IS NULL",
				mi_msecs, period,mi_function);
	} else {
		s3_MustDof(s3Usage, "INSERT INTO usage (calls,duration,period,api,caller) VALUES (1,%d,%s,%s,NULL)",
				mi_msecs, period,mi_function);
	}

	s3_Queryf(s3Usage, "SELECT COUNT(*) count FROM usage WHERE period=%s AND api IS NULL AND caller=%s",
			period, mi_caller);
	if (count) {
		s3_MustDof(s3Usage, "UPDATE usage SET calls=calls+1, duration=duration+%d WHERE period=%s AND api IS NULL AND caller=%s",
				mi_msecs, period,mi_caller);
	} else {
		s3_MustDof(s3Usage, "INSERT INTO usage (calls,duration,period,api,caller) VALUES (1,%d,%s,NULL,%s)",
				mi_msecs, period,mi_caller);
	}
	...
	if (!_bNoteInhibit) {
		...
		if (lastio != io) {							// We're sending to someone new
			if (fpnote) {
				...
				FileDescription(szFilename, "Data sent");
				szDelete(szFilename);

				Note("M|%s.%s|%s|%d", szBase, "txt", "Data sent", -1);
			}
			if (fp) fputs("\n----------------------------------------------------------- NEW PORT\n", fp);
			lastio=io;
		}
...
void msc_DisplayMsgLogLine(BIO *io, const char *dir, int bDeep, const char *szFilter, int day, int month, int year)
{
	...
	if (bInclude) {
		...
		BIO_putf(io, "<td nowrap=1><a href=\"/getinteraction/%s\">%s</a>", dir, dir);
		const char *rcv = hprintf(NULL, "msglog/%s/rcv.xml", dir);
		if (!access(rcv, 4)) {
			BIO_putf(io, "<td><a href=\"/rerunform/%s\">Rerun</a></td>", dir);
		} else {
			MyBIO_puts(io, "<td></td>");
		}
		szDelete(rcv);
		...
		if (!access(szInputFile, 0)) {		// It's called rpc-in.xml
			BIO_putf(io, "<td nowrap=1><a href=\"/getfile/%s/rpc-in.xml\">%s</a></td>", dir, szZInfo, szZInfo);
		} else {							// It's called rcv.xml
			BIO_putf(io, "<td nowrap=1><a href=\"/getfile/%s/rcv.xml\">%s</a></td>", dir, szZInfo, szZInfo);
		}
		szDelete(szInputFile);

		// Returned
		if (nCode >= 0) {
			BIO_putf(io, "<td nowrap=1>%d (<a href=\"/getfile/%s/rpc-out.xml\">%s</a>)</td>", nCode, dir, szReturn);
		} else {
			BIO_putf(io, "<td nowrap=1><a href=\"/getfile/%s/rpc-out.xml\">%s</a></td>", dir, szReturn);
		}
		...
		if (!access(szStderrFile, 0)) {		// We have stderr text
			BIO_putf(io, "<td nowrap=1><a href=\"/getfile/%s/stderr.txt\" title=\"%s\">%s</a></td>", dir, szStderr, szErrSize);
		} else {
			BIO_putf(io, "<td>");
		}
		szDelete(szStderrFile);
		BIO_putf(io, "<td><a href=\"/status?f=%s\">%s</a></td>", szIP, szIP);
...
void xyzzymsc_DisplayMsgLogLine(BIO *io, const char *dir, int bDeep, const char *szFilter, int day, int month, int year)
...
//BIO_putf(io, "Looking in %s/%s for bollocks", dir, "log");
	...
	int bErr = msc_HasLogError(szLogFile);
	szDelete(szLogFile);
	...
	BIO_putf(io, "<td nowrap=1>%s</td>", msc_FilterDayLink(day, month, year, szEnv));
	BIO_putf(io, "<td nowrap=1>%s</td>", msc_FilterDayLink(day, month, year, szFrom));
	BIO_putf(io, "<td nowrap=1>%s</td>", msc_FilterDayLink(day, month, year, szTo));
	BIO_putf(io, "<td nowrap=1>%s</td>", msc_FilterDayLink(day, month, year, szNhs));
	if (*szUuid == '-') {
		BIO_putf(io, "<td>-</td>");
	} else {
		BIO_putf(io, "<td class='TD_MSGID'>%s</td>", msc_FilterDayLink(day, month, year, szUuid));
	}
	if (*szConv == '-') {
		BIO_putf(io, "<td>-</td>");
	} else {
		BIO_putf(io, "<td class='TD_MSGID'>%s</td>", msc_FilterDayLink(day, month, year, szConv));
	}
	BIO_putf(io, "<td class='TD_%s' nowrap=1>%s</td>", szProject, msc_FilterDayLink(day, month, year, szProject));
	if (szZInfo) {
		BIO_putf(io, "<td nowrap=1 colspan=2>%s</td>", msc_FilterDayLink(day, month, year, szZInfo));
	} else {
		BIO_putf(io, "<td nowrap=1>%s</td>", msc_FilterDayLink(day, month, year, szType));
	}
	BIO_putf(io, "<td>%s</td>", msc_FilterDayLink(day, month, year, szIP));
...
//Log("Done getc read nGot=%d, nResult = %d, c='%c', errno=%d, flags=%x", nGot, nResult, (c>=20&&c<=126)?c:'-', errno, io->flags);
	...
	if (!_bNoteInhibit) {
		...
		if (lastio != io) {						// Changed to a new thrower
			...
			if (fpnote) {
				const char *szFilename = hprintf(NULL, "%s.txt", szBase);
				FileDescription(szFilename, "Data received");
				szDelete(szFilename);

				Note("M|%s.%s|%s|%d", szBase, "txt", "Data received", -1);
			}
			lastio=io;
		}
...
static void TableLine(BIO *io, const char *title, const char *value)
{
	if (!value) value="Unknown";

	BIO_putf(io, "<tr><td bgcolor=#ffaa88>%s<td bgcolor=#ffffaa>%s\r\n", title, value);
}
...
static void TableNotes(BIO *io, note_t *note, const char *szDir)
{
	...
	if (info) {
		...
		TableLine(io, "Function", ssmap_GetValue(info, "API-FUNCTION"));
		TableLine(io, "Date-Time", ssmap_GetValue(info, "TIME-START"));
		TableLine(io, "Directory", szDir);
		TableLine(io, "Session", ssmap_GetValue(info, "MI-SESSION"));
		TableLine(io, "Caller", ssmap_GetValue(info, "MI-CALLER-ID"));
		TableLine(io, "Status", ssmap_GetValue(info, "MI-STATUS"));
		TableLine(io, "Calling method", ssmap_GetValue(info, "CLIENT-TYPE"));
		BIO_putf(io, "</table>\r\n");
	} else {
		BIO_putf(io, "<table bgcolor=#000077 bordercolor=blue border=1 cellspacing=1 width=100%>\r\n");
...
static void TableFiles(BIO *io, const char *szDir)
{
	...
	if (map) {
		...
		while (ssmap_GetNextEntry(map, &szFile, &szDescription)) {
			...
			BIO_putf(io, "<td><a href=\"/getfile/%s/%s\">%s</a></td>", szDir, szFile, szDescription);
...
// $mtorange="#f57023"; $mtblue="#2b509a";
	...
	BIO_putf(io, 
		...
		"<tr><td rowspan=2 colspan=2 align=center><font size=6 color=white>SPIDER v" VERSION " - RPC Server (" OS ")</font>"
...
//Log("ParseAddress(%s,%s,%s,%s)", szURI, szAddr, "...", "...");
	if (!strcasecmp(szAddr, "/rpc") || !strncasecmp(szAddr, "/rpc/", 5)) {
		...
		if (!cbacId && !isPromiscuous()) FatalHttp(io, 403, "Access denied to %s:%s", szOrganisation, szProduct);
