...
static const char *szRpcError = NULL;				// Error returned from RPC call
...
// NoteRpcMessage(szRpcError, -1, "stderr", "txt", "Stdout returned from RPC handler");
{
	if (!_bNoteInhibit && szRpcError) {
		...
		fp=fopen(szFilename, "w");
		if (fp) {
			int len = strlen(szRpcError);
			fwrite(szRpcError, len, 1, fp);
			fclose(fp);
...
// Writes the management information to a file
{
	...
	if (fp) {
		...
		fprintf(fp, " %0.2f", (double)(mi_tms.tms_utime+mi_tms.tms_stime)/CLK_TCK);			// Spider time
		fprintf(fp, " %0.2f", (double)(mi_tms.tms_cutime+mi_tms.tms_cstime)/CLK_TCK);		// API time
...
static const char *HttpHeader(nCode)
{
	...
	mi_status = nCode;
	result = hprintf(NULL, "HTTP/1.1 %s %s\r\n", nCode, HttpResponseText(nCode));

	return result;
}

static void SendHttpHeader(BIO *io, int nCode, const char *szContentType, const char *szContent)
{
...
//		X509 *certPeer = SSL_get_peer_certificate(ssl);
		...
		if (strstr(szFilename, "/../")) {
			SendHttpHeader(io, 403, NULL, NULL);
		...
		if (!got) {
			SendHttpHeader(io, 404, NULL, NULL);
...
//SendHttpHeader(io, 200, "text/plain", NULL);
//SendHtmlHeader(io);
...
//			rogxml*	An error created using 'ErrorMessage()'
{
	...
	const char *szSupplier = "Acme";
	const char *szApp = "Coyote";
	const char *szVersion = "1.00";
	const char *szKey = "Yale";

	if (s3) {
		long enabled;
		s3_VarLong(s3, "enabled", &enabled);
//		S3LONG(s3,"enabled")

		s3it *it = s3_Queryf(s3, "SELECT enabled "
				" FROM authorisation "
				" WHERE supplier=%s, app=%s, version=%s AND key=%s",
		...
		if (it) {
			if (s3it_Next(it)) {
				...
				s3it_Destroy(&it);
			} else {					// Not found in the table
				s3it_Destroy(&it);
				long count;
				s3_VarLong(s3, "count", &count);
//				S3LONG(s3,"count");
				it = s3_Queryf(s3, "SELECT COUNT(*) as count "
					" FROM authorisation "
					" WHERE supplier=%s, app=%s",
					szSupplier, szApp);
				s3it_Next(it);
				if (count >= 1) {
					szErr = hprintf(NULL, "Application %s/%s, version %s not recognised", szSupplier, szApp, szVersion);
				} else {
					szErr = hprintf(NULL, "Application %s/%s unrecognised", szSupplier, szApp);
				}
			}
		}
		s3_Close(&s3);
