...
static char g_isSecure = 0;					// 1 if we have a secure (HTTPS) connectino
...
int legacyChild = 0;							// pid of child that deals with legacy API calls
int legacyReady = 0;							// 1 when legacy child is ready to accept calls
CHAN *main_channel_pool = NULL;					// Channel list used in ws_EventLoop()
...
SPMAP *contracts = NULL;				// Points to 'contract_t's, indexed on organisation:product
...
HLIST *hl_NoteLog = NULL;
HLIST *hl_Stdout = NULL;

SPMAP *hl_Files = NULL;					// List of miscellaneous files to write
...
//
char msglog_KeepFlows = 0;			// 1 if we're going to keep dataflows (files that start 'flow')
char msglog_KeepErrors = 1;			// 1 if we're going to set msglog_Keep on meeting an error
char msglog_KeepAlways = 0;			// 1 if we're going to keep everything always

char makeCallHtml = 1;					// call.html gets generated when invoking an API
...
S3 *s3Main()
{
return NULL;				// Hopefully turn off the handling
	if (!s3MainDb) {
...
const char *service_DatabaseFile()
{
	static const char *file = NULL;

	if (!file) {
		file = hprintf(NULL, "%s/%s", szEtcDir, "service.sl3");
	}
...
{
	if (!_s3_Service) {
		_s3_Service = s3_Open(service_DatabaseFile());

		s3_CreateTable(_s3_Service, "listeners",
				...
				" uri TEXT, "
				" protocol TEXT, "
				" verb TEXT DEFAULT \"POST\", "
				" expires DATETIME, "
				" UNIQUE (service,ip,port), "
				" UNIQUE (ip,id) "
				);
	}
...
int cbac_CanSeeApi(const char *szOrganisationProduct, const char *szApi)
...
const char *cbac_CanRunApi(const char *szOrganisationProduct, const char *szApi)
...
void contract_DumpAll()
{
	if (contracts) {
		...
		while (spmap_GetNextEntry(contracts, &name, (void**)&c)) {
...
//
// WARNING - if you change this 11 character requirement, you'll need to change such things as msc_NoteDirTail()
...
HLIST *NoteList(const char *filename)
...
static void NoteLog(const char *szText)
{
	...
	if (!hl_NoteLog) {
		...
		hlist_AddHeap(hl_NoteLog, -1, hprintf(NULL, "Spider %s for %s (Made %s on %s, using %s)\n",
				VERSION, OS, __TIME__, __DATE__, SSLeay_version(SSLEAY_VERSION)));
	}
...
//				Log("API Debug (%s): %d byte%s", NoteFileAnchor(api_stderr, "stderr"), bytes, bytes==1?"":"s");
...
//Log("Alarm %d has gone off", _nAlarmNumber);
		...
		return;											// False alarm...
		break;
	case SIGUSR1:										// 16 Child process is ready for legacy connections
		legacyReady = 1;
		return;
		break;
...
#if 0		// Taken care of using flow_spider_client now
	const char *szNoteName = hprintf(NULL, "toapp%d.xml", nNumberSent);
	if (nSendErr) {
...
void LogBuffer(const char *message, int len, const char *data)
{
	...
	Log("%s (%d) = '%s'", message, len, buf);
	free(buf);
}

int HandleIncomingUtf8(WS *channel, int len, const char *text)
...
{
Log("WS-IN(%s): %.*s", channel->id, len, text);
...
void OnInvoked(WAMP *wamp, long long requestId, const char *procedure, JSON *list, JSON *dict)
...
{
	Log("OnInvoked(%s...)", procedure);

	JSON *yield = json_NewArray();

	json_ArrayAddInteger(yield, WAMP_YIELD);
	json_ArrayAddInteger(yield, requestId);
	json_ArrayAdd(yield, json_NewObject());
	JSON *resultArray = json_NewArray();
	json_ArrayAddString(resultArray, -1, "I am the answer");
	json_ArrayAdd(yield, resultArray);
//	if (resultDict) json_ArrayAdd(yield, resultDict);
	wamp_WriteJsonHeap(wamp, yield);
}

void BecomeLegacyHandler(int fd)
// Become a WAMP handler for all legacy calls
// We talk through fd.
// We never return.
{
	CHANPOOL *pool = chan_PoolNew();								// Create a pool of connections for polling
	CHAN *channel = chan_New(pool, fd, NULL, WS_IN | WS_OUT);		// Create a connection for comms with parent
	WS *ws = ws_New(channel);										// Create a new websocket connected to the channel
//	ws_SetUtf8Handler(ws, HandleIncomingUtf8);						//

	WAMP *wamp = wamp_NewOnWebsocket(ws);							// Add a wamp client with the websocket

	wamp_SetRealm(wamp, "spider");

	// All of the hello and registering us as a callee has already been done by our parent

	wamp_RegisterInvokee(wampenv, OnInvoked);
	chan_EventLoop(pool);

	exit(0);
}

int CreateLegacyChild()
{
	int legacyfds[2];						// Parent will talk through [0], child through [1]
	pipe(legacyfds);

	if (legacyChild = fork()) {				// Parent
		close(legacyfds[1]);
Log("Parent to handle legacy on %d", legacyfds[0]);
		ws_NewChannel(main_channel_pool, legacyfds[0], NULL, WS_IN | WS_OUT);			// Add channel to chat with the child
	} else {									// I am the legacy child process
Log("Child to handle legacy on %d", legacyfds[1]);
		close(legacyfds[0]);
		BecomeLegacyHandler(legacyfds[1]);		// Never returns
	}
}

int wampcb_NoCallee(const char *procedure)
{
	Log("We have tried to call '%s'", procedure);

	if (!strncmp(procedure, "api.", 4)) {
		if (legacyChild) {
		...
		if (legacyChild = fork()) {				// Parent
			close(legacyfds[1]);
Log("Parent to handle legacy on %d", legacyfds[0]);
			WS *channel = ws_NewChannel(main_channel_pool, legacyfds[0], NULL, WS_IN | WS_OUT);
			WAMP *wamp = wamp_GetForChannel(channel);

			JSON *hello = json_NewArray();						// Register the new child
			json_ArrayAddInteger(hello, WAMP_HELLO);
			json_ArrayAddString(hello, -1, "spider");
			...
			JSON *jRegister = json_NewArray();					// Say the new child can deal with api. calls
			json_ArrayAddInteger(jRegister, WAMP_REGISTER);
			json_ArrayAddInteger(jRegister, 1234);
			JSON *registerOpts = json_NewObject();
			json_ObjectAddString(registerOpts, "match", -1, "prefix");
			json_ArrayAdd(jRegister, registerOpts);
			json_ArrayAddString(jRegister, -1, "api.");
...
int FailWebsocket(BIO *io, const char *reason)
{
...
static int DealWithWebsocket(BIO *io, const char *szURI, MIME *header)
{
	...
	ws_SetUtf8Handler(main_channel_pool, HandleIncomingUtf8);
	WS *c = ws_NewChannel(main_channel_pool, -1, io, WS_IN | WS_OUT);

	return ws_EventLoop(main_channel_pool);
}
...
static int DealWithGET(BIO *io, const char *szURI)
{
	...
	if (szConnection && !stricmp(szConnection, "upgrade")) {
		...
		if (szUpgrade && !stricmp(szUpgrade, "websocket")) {
			return DealWithWebsocket(io, szURI, mime);
		}
	}
...
BIO *BuildConnection(const char *szAddress, int nPort, int isSsl)
// isSsl = 1 to make an SSL connection
{
	BIO *io_socket;								// The way SSL talks to the port
	BIO *io_buffer;								// Buffered BIO - the way we talk through the SSL
	int nErr;

//Log("BuildConnection(%s, %d, %d)", szAddress, nPort, isSsl);
	// Create the SSL/TLS connection to the other side
...
//Log("Opening socket");
	...
	if (isSsl) {									// Go through all the SSL stuff
...
int HttpRequest(const char *szAddr, int nPort, int isSsl, const char *verb, const char *uri, SSMAP *headers, const char *content, int len)
// Send a request to a server and return a result.
// The header returned by the remote server will be placed in *pHeader
{
Log("HttpRequest(%s,%d,%d,%s,%s,%p,%s,%d)", szAddr, nPort, isSsl, verb, uri, headers, content, len);
	BIO *io = BuildConnection(szAddr, nPort, isSsl);
	if (!io) return 1;

	int posting = !stricmp(verb, "POST");

	if (!uri || !*uri) uri="/";
	if (!content) content="";
	if (len == -1) len=strlen(content);
	if (!headers) headers=ssmap_New();
	ssmap_Add(headers, "Host", szAddr);

	const char *myUri = strdup(uri);
//	if (!posting && len) {
//		myUri = hprintf(myUri, "?%s", content);
//		len=0;
//	}
	...
	if (len) {
		...
		ssmap_Add(headers, "Content-Length", buf);
	}

	BIO_putf(io, "%s %s HTTP/1.1\r\n", verb, myUri);
Log("%s %s HTTP/1.1", verb, myUri);
	if (headers) {
		const char *name;
		const char *value;

		ssmap_Reset(headers);
		while (ssmap_GetNextEntry(headers, &name, &value)) {
			BIO_putf(io, "%s: %s\r\n", name, value);
Log("%s: %s", name, value);
		}
	}
...
int HttpRequestValues(const char *szAddr, int nPort, int isSsl, const char *verb, const char *uri, SSMAP *headers, SSMAP *values, const char *payload)
{
Log("HttpRequestValues(%s,%d,%d,%s,%s,%p,%p,%s)", szAddr, nPort, isSsl, verb, uri, headers, values, payload);
	ssmap_Reset(values);
	const char *name;
	const char *value;
	...
	char sep='?';
	while (ssmap_GetNextEntry(values, &name, &value)) {
		const char *uName = msc_UriEncode(name);
		const char *uValue = msc_UriEncode(value);

		myUri = hprintf(myUri, "%c%s=%s", sep, uName, uValue);
	...
	int result = HttpRequest(szAddr, nPort, isSsl, verb, myUri, headers, payload, -1);
	szDelete(content);
...
const char *rogxml_GetAttrDef(rogxml *rx, const char *szAttr, const char *szDef)
{
	const char *szValue = rogxml_GetAttr(rx, szAttr);

	return szValue ? szValue : szDef;
}

void ApiNeeds(const char *szName, const char *szValue)
{
	if (!szValue) FatalHttp(bio_err, 500, "API needs parameter of %s", szName);
}

const char *MustParam(rogxml *rx, const char *szName)
{
	const char *szValue = rogxml_GetAttr(rx, szName);

	ApiNeeds(szName, szValue);

	return szValue;
}

void service_Send(const char *ip, int port, const char *service, const char *uri, const char *protocol, const char *verb, SSMAP *values, const char *payload)
// protocol MUST be HTTP or HTTPS
// verb MUST be POST or GET
{
Log("service_Send(%s,%d,%s,%s,%s,%s,%p,%s)", ip, port, service, uri, protocol, verb, values, payload);
	const char *content = "Hello";
	SSMAP *headers = ssmap_New();

	int isSsl = !stricmp(protocol, "HTTPS");
	HttpRequestValues(ip, port, isSsl, verb, uri, headers, values, payload);
}

static rogxml *service_DoListener(rogxml *rxRpc)
{
	S3 *db = service_Database();

	const char *szService = rogxml_GetAttr(rxRpc, "service");
	const char *szProtocol = rogxml_GetAttrDef(rxRpc, "protocol", "HTTP");
	const char *szVerb = rogxml_GetAttrDef(rxRpc, "verb", "POST");
	const char *szUri = rogxml_GetAttrDef(rxRpc, "uri", "/");
	const char *szIp = rogxml_GetAttr(rxRpc, "ip");
	int nPort = rogxml_GetAttrInt(rxRpc, "port", 0);

	ApiNeeds("service", szService);
	ApiNeeds("ip", szIp);
	if (!nPort) ApiNeeds("port", NULL);

	int err = s3_MustDof(db, "INSERT INTO listeners (service,callerip,ip,port,uri,protocol,verb,expires) "
	...
	Log("Got: %s, err = %d", rogxml_ToText(rxRpc), err);
	if (err) Log("Error: %s", s3_ErrorStr(db));
	return rogxml_NewError(999, "Yes, I got here ok");
}

void service_BroadcastApiResult(const char *szApi, const char *szResult)
{
	S3 *db = service_Database();

	const char *szService = hprintf(NULL, "api.%s", szApi);

	S3STRING(db,ip);
	S3LONG(db,port);
	S3STRING(db,uri);
	S3STRING(db,protocol);
	S3STRING(db,verb);
	S3STRING(db,expires);
	S3STRING(db,callerip);

	s3it *it = s3_MustQueryf(db, "SELECT callerip,ip,port,uri,protocol,verb,expires FROM listeners WHERE service=%s", szService);
	while (s3it_Next(it)) {
		Log("Broadcasting %s to %s:%d", szService, ip, port);
		if (!fork()) {
			SSMAP *values = ssmap_New();

			ssmap_Add(values, "source", callerip);
			ssmap_Add(values, "service", szService);
			service_Send(ip, port, szService, uri, protocol, verb, values, szResult);
			exit(0);
		}
//		Log("I have (%s,%d,%s,%s,%s,%s)", ip, port, uri, protocol, verb, expires);
	}
}

static rogxml *service_DoListen(rogxml *rxRpc)
{
	return rogxml_NewError(999, "Yes, I got here ok");
}

static rogxml *service_DoIgnore(rogxml *rxRpc)
{
	S3 *db = service_Database();
	...
	if (szService) {
		if (nPort) {
			err = s3_MustDof(db, "DELETE FROM listeners WHERE service=%s AND ip=%s AND port=%d", szService, szIp, nPort);
		} else {
			err = s3_MustDof(db, "DELETE FROM listeners WHERE service=%s AND ip=%s", szService, szIp);
		}
	} else {
		if (nPort) {
			err = s3_MustDof(db, "DELETE FROM listeners WHERE ip=%s AND port=%d", szIp, nPort);
		} else {
			err = s3_MustDof(db, "DELETE FROM listeners WHERE ip=%s", szIp);
		}
	}
...
static rogxml *service_DoSend(rogxml *rxRpc)
{
	return rogxml_NewError(999, "Yes, I got here ok");
}

static rogxml *service_DoBroadcast(rogxml *rxRpc)
{
	S3 *db = service_Database();

	const char *szService = rogxml_GetAttr(rxRpc, "service");
	const char *szPayload = rogxml_GetAttr(rxRpc, "payload");
	...
	while (s3it_Next(it)) {
		...
		if (!fork()) {
			...
			ssmap_Add(values, "source", callerip);
			ssmap_Add(values, "service", szService);
			service_Send(ip, port, szService, uri, protocol, verb, values, szPayload);
			exit(0);
		}
//		Log("I have (%s,%d,%s,%s,%s,%s)", ip, port, uri, protocol, verb, expires);
	}
...
static rogxml *service_DoFunction(rogxml *rxRpc, const char *szFunction)
{
	if (!stricmp(szFunction, "listener")) return service_DoListener(rxRpc);
	else if (!stricmp(szFunction, "listen")) return service_DoListen(rxRpc);
	else if (!stricmp(szFunction, "ignore")) return service_DoIgnore(rxRpc);
	else if (!stricmp(szFunction, "send")) return service_DoSend(rxRpc);
	else if (!stricmp(szFunction, "broadcast")) return service_DoBroadcast(rxRpc);
	else return rogxml_NewError(410, "Function spider.service.%s not recognised", szFunction);
}
...
{
	rogxml *rxResultSet = NewResult();
	const char *dot = strchr(szFunction, '.');
...
//				rogxml_SetAttr(rx, "binary",api_Binary(*l));
	...
	} else if (dot && !strnicmp(szFunction, "service", dot-szFunction)) {	// spider.service.*
		rxResultSet = service_DoFunction(rxRpc, dot+1);
...
void ReportUsage()
// Report on child resource usage
{
	struct rusage ru;

	int err = getrusage(RUSAGE_CHILDREN, &ru);
	if (err) {
		Log("Cannot get usage %d (%s)", errno, strerror(errno));
		return;
	}

	Log("utime = %d.%d", ru.ru_utime.tv_sec, ru.ru_utime.tv_usec);
	Log("stime = %d.%d", ru.ru_stime.tv_sec, ru.ru_stime.tv_usec);
	Log("Swaps = %d", ru.ru_nswap);
	Log("Page faults = %d, %d", ru.ru_minflt, ru.ru_majflt);

	struct tms tms1;
	long tim1 = times(&tms1);
	Log("Times %d %d %d %d %d", tim1, tms1.tms_utime, tms1.tms_stime, tms1.tms_cutime, tms1.tms_cstime);

	struct timeval tp;
	gettimeofday(&tp, NULL);
	int usecs=tp.tv_usec;
	Log("Timeofday = %d.%d", tp.tv_sec, tp.tv_usec);
	Log("Now = %f", GetNow());
	Log("Now = %f", GetNow());
}
...
//	Log("API Call (%s): %d bytes", NoteFileAnchor(flow_api_call, flow_api_call), hlist_Length(hl_api_call));
	...
	fcntl(fd0[0], F_SETFL, O_NONBLOCK);					// We don't want to block writing to the child stdin

	double apiStartTime = GetNow();

	if (!(pid = fork())) {								// Child process
		close(fd0[0]);									// Close the parent ends of the pipes
		close(fd1[0]);

		/* Map STDIN and STDOUT to the pipe ends and execute */
		if( fd0[1] != 0 ) {								// I'm not sure how fd0[0] could possibly have been 0 - existing code
			if( dup2( fd0[1], 0) == -1 ) {				// Dup the child end of pipe 0 onto stdin
				char* err = strerror(errno);
				Log("RPC aborted - Error copying pipe for stdin: %s", err);
				exit(5);
			}
			close(fd0[1]);								// Close the old pipe end
		}
		if( fd1[1] != 1 ) {								// Again, I don't see how this pipe could have been created as 1
			if( dup2( fd1[1], 1 ) == -1 ) {				// Dup the child end of pipe 1 onto stdout
				char* err = strerror(errno);
	...
	close( fd0[1] );									// Close off the remote ends of the pipes
	close( fd1[1] );

//	fcntl(fds[0].fd, F_SETFL, O_NONBLOCK);				// We don't want to block writing to the child stdin
	fds[0].fd = fd0[0];									// Prepare the array that poll() uses to monitor the pipes
	fds[1].fd = fd1[0];
...
//		Log("API Input (%s): %d byte%s", NoteFileAnchor(flow_spider_api, flow_spider_api), stdin_len, stdin_len==1?"":"s");
...
void ReturnResult(BIO *io, int bWantHeader, const char *szApi, rogxml *rxResult)
{
	if (rxResult) {
		...
		if (szContent) {
			...
			if (fd >= 0) {
				...
				Log("Returning content in %s (%Ld byte%s)", szContent, len, len==1?"":"s");
				lseek64(fd, 0, 0);
				...
				const char *note = hprintf(NULL, "{ contents of '%s' (%Ld byte%s) }", szContent, len, len==1?"":"s");
...
// Commented out so as not to impact on any regression testing
//			service_BroadcastApiResult(szApi, szResult);
		}
	} else {
...
int GetIncomingHeader(BIO *io, const char **pMethod, const char **pURI, const char **pVersion)
...
static void ProcessHttp(BIO *io, const char *szMethod, const char *szURI, MIME *mime)
{
	Log("Processing a %s on %s", szMethod, szURI);

	if (strcasecmp(szMethod, "OPTIONS")) {
		Log("I've got an options");
	}
	MyBIO_flush(io);
}

static void ProcessIncomingConnection(BIO *io)
{
	const char *szMethod;
	const char *szURI;
	int keepAlive = 1;
	...
	mime_ParseHeaderValues(0);				// Don't want to parse header values
	while (keepAlive && GetIncomingHeader(io, &szMethod, &szURI, NULL)) {
		keepAlive=0;						// Pessimistic to the end
		MIME *mime = GetIOMIME(io);

		if (mime) {
			ProcessHttp(io, szMethod, szURI, mime);
			const char *szConnection = mime_GetHeader(mime, "Connection");
			if (!strcasecmp(szConnection, "Keep-Alive"))
				keepAlive=1;
			mime_Delete(mime);
		} else {
			Log("No MIME came with a '%s %s HTTP'", szMethod, szURI);
		}
	}

	Log("There endeth the connection");
}

void LogMap(SSMAP *map)
{
	const char *name;
	const char *value;
...
const char *GetHttpBody(BIO *io, SSMAP *header, unsigned long *pLen)
{
	char *buf=NULL;
...
SSMAP *GetHttpRequest(BIO *io, const char **pBody, unsigned long *pLen)
{
...
SSMAP *GetFullHttpRequest(BIO *io, const char **pMethod, const char **pURI, const char **pBody, unsigned long *pLen)
// Gets a complete HTTP request including the first line
{
	SSMAP *headerMap;

	int nGot = GetIncomingHeader(io, pMethod, pURI, NULL);
	if (nGot) {
		headerMap = GetHttpRequest(io, pBody, pLen);
	}

	return headerMap;
}

int WantKeepAlive(SSMAP *header)
...
const char *webdav_BaseDir()
// Should be dependent on environment and/or other factors but static while I experiment
{
	return "/usr/mt/spider";
}

const char *webdav_InternalFromExternal(const char *szExternal)
...
// Returns an effective static.
{
	...
	if (szExternal) {
		...
		result = hprintf(NULL, "%s/%s", webdav_BaseDir(), szExternal);
	}

	return result;
}

int webdav_AddSupportedLock(rogxml *prop)
...
int webdav_Response(rogxml *xml, const char *szExternal)
...
//Log("webdav_Response(%s) - is a directory", szExternal);
			...
			rogxml_AddTextChildf(prop, "d:displayname", "%s", szLeaf);
...
//Log("webdav_Response(%s) - is a file", szExternal);
			...
			rogxml *resourcetype = rogxml_AddChild(prop, "d:resourcetype");
//			rogxml_AddChild(prop, "d:lockdiscovery");
			webdav_AddSupportedLock(prop);
			rogxml_AddTextChild(propstat, "d:status", "HTTP/1.1 200 OK");
			result = 1;
		} else {
...
rogxml *webdav_DirProperties(const char *szExternal, int depth)
...
//Log("webdav_DirProperties(%s, %d) - entryType(%s) = %d", szExternal, depth, szInternal, entryType);
	...
	if (entryType == 2 && depth > 0) {
		...
		files=GetDirEntries(files, szInternal, szInternal, NULL);
		if (files) {
			const char **f=files;
			while (*f) {
				Log("Found file: %s", *f);
...
//	LogMap(header);
	...
	if (xml) {
		...
		char szLen[10];
		snprintf(szLen, sizeof(szLen), "%d", strlen(szXml));
...
int HttpLoop(BIO *io, const char *szMethod, const char *szURI)
...
// At this point, if buf[0] == 127, we have a RawSocket connection.
...
const char *spider_Call(const char *szHost, const char *szSessionId, const char *szSessionToken, const char *szApi, const char *szzParams, int nInputLen, const char *szInput, const char *szOrganisationProduct)
...
rogxml *spider_CallXml(const char *szHost, const char *szSessionId, const char *szSessionToken, const char *szApi, const char *szzParams, int nInputLen, const char *szInput, const char *szOrganisationProduct)
{
...
int spider_SessionCreate(const char *szHost, const char **pSessionId, const char **pSessionToken, rogxml **pXml, const char *szOrganisationProduct)
{
...
int spider_Login(const char *szHost, const char *szSessionId, const char *szSessionToken, const char *szSite, const char *szUser, const char *szPassword, const char *szOrganisationProduct)
{
	char params[100];
...
void CallSpider(const char *szHost, const char *szSessionData, const char *szLoginData, const char *szApi, char *argv[], const char *szInputFile, const char *szOrganisationProduct)
{
	int nSock;
...
void InstallApi(const char *szFilename, int bForce)
{
	char cwd[512];
	...
	const char *szBinary = BinaryFromApi(szName);
	const char *szExistingVersion = "0.0";
	int bExists = 0;

	if (szBinary) {										// It already exists
		bExists=1;
