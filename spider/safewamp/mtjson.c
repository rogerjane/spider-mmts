		...
		if (!(c & 0x80)) {									// 7-bit characters
			...
			if (codepoint >= 0) {							// Happily found our codepoint
				...
				if (codepoint <= 0xffff) {
					...
					sprintf(buf, "\\u%04x", 0xd800 + (codepoint >> 10));
					hbuf_AddBuffer(h, 6, buf);
					sprintf(buf, "\\u%04x", 0xdc00 + (codepoint & 0x3ff));
					hbuf_AddBuffer(h, 6, buf);
				}
			} else {										// Illegal UTF-8...
...
API const char *json_RenderHeap(JSON *json)
...
// Passing in a NULL results in a blank string.
{
	...
	if (json) {
		...
		case JSON_ARRAY:
			...
			for (i=0;i<json->data.array.count;i++) {
				if (i) hbuf_AddChar(h, ',');
				hbuf_AddHeap(h, -1, json_RenderHeap(json->data.array.values[i]));
			}
			hbuf_AddChar(h, ']');
			break;
		case JSON_OBJECT:
			...
			if (json->data.object) {				// May be NULL if json_New(JSON_OBJECT) called, but map not initilised yet
				spmap_Reset(json->data.object);
				const char *name;
				JSON *value;
				while (spmap_GetNextEntry(json->data.object, &name, (void**)&value)) {
					...
					hbuf_AddChar(h, ':');
					hbuf_AddHeap(h, -1, json_RenderHeap(value));
				}
			}
			hbuf_AddChar(h, '}');
			break;
...
// As json_Render but the returned string is managed by this function (so don't go free()ing it!)
// If you want to de-allocate the memory, call this function with NULL.
{
	static const char *result = NULL;
	szDelete(result);

	result = json_RenderHeap(json);
