...
Log("WA: wamp_OnHttpResponse(%s) - got %d = '%.*s', found crlfcrlf = %d", chan_Name(chan), got, got<20?got:20, peek, crlfcrlf?1:0);
...
Log("WA: Response isn't long enough (%d)", got);
		err = 1;
	}

	if (!err && strncmp(header, "HTTP/1.1 101 ", 13)) {		// ERR - MUST return a 101 response
Log("WA: Expecting 'HTTP/1.1 101 ' and didn't see it");
		err = 2;
	}
...
Log("WA: Header line doesn't end");
			err = 3;
		}
...
Log("WA: Warning - I've ignored a line with no colon in header returned from peer WAMP server");
				}
				line=nextLine;
			}
		}
		...
		if (!err) {
			...
			if (responseError) {
Log("WA: WAMP response error: %s", responseError);
				err = 4;
			} else {
...
Log("WA: Successfully made outgoing connection to another WAMP server - added to the collective");
			}
		}
	}
