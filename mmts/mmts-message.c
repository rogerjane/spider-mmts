...
// Returns the result and stores it in the message
{
	...
	if (!strcmp(szURI, "http://schemas.xmlsoap.org/soap/envelope/") &&		// In SOAP namespace
		...
		if (rxChild) {
			...
			if (szHeaderURI && (!strcmp(szHeaderURI, "hl7") || !strcmp(szHeaderURI, "urn:hl7-org:v3"))) { // Directly embedded HL7 (NASP)
...
//Log("msg_ExtractMessageInfo(%x) - WRAP_EBXML", m);
		...
		if (!szRefToMessageId && (!szMessageId || !szConversationId || strcasecmp(szMessageId, szConversationId))) {
...
//////
STATIC int GetHeader(FILE *fp, const char *szNameCheck, char const **pszName, char const **pszType, int *pnNum)
...
STATIC const char *GetString(FILE *fp)
{
	...
	if (GetHeader(fp, NULL, &szName, &szType, &nLen)) {
		if (nLen >= 0) {
			szText=(char *)malloc(nLen+1);
			itemsRead = fread(szText, 1, nLen, fp);
			szText[itemsRead]='\0';
			if( (int)itemsRead == nLen )
