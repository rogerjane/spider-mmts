...
const char *GetBaseDir()				{ return szBaseDir ? szBaseDir : "BaseDirNotSet"; }
...
int PeerPort(int bExt) { return _nSenderPort; }
...
const char *SignalName(int n)
{
	...
	if (n >= 0 && n<(int)(sizeof(names)/sizeof(*names)))
...
static const char *logPrefix = "BOOTER";
...
{
	if (!szInteractionId) szInteractionId = "";
	...
	if (!szSection) {												// Not found anything in /usr/mt/mmts/env/interactions
		const char *colon = strchr(szInteractionId, ':');			// Is this perhaps a 'fhir' type interaction?

		if (colon) {												// We'll use the part up to the colon, but need to be careful
			static const char *fhirType = NULL;						// Keep a safe place on the heap

			szDelete(fhirType);
			fhirType = strnappend(NULL, szInteractionId, colon-szInteractionId);	// Just the part before the ':'
			szSection = fhirType;
		}
	}
		...
		while ((got=fread(buf, 1, sizeof(buf)-1, p))) {
			...
			while (*bufp) {
				...
				if (chp) {												// We have a tail end
					...
					bufp=bufp+strlen(bufp);								// Force breaking from the loop
				}
			}
