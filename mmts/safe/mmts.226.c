...
// TODO: Consider handing off requests to spider that come via HTTPS port
// TODO: when a child finishes, add a one-line entry to a log summarising it (include EPS id, NHS no etc. where available)
// TODO: Try to do away with the need for 'overrides'...!
// TODO: Make all ldap cache entries a GUID, using the whole query string
// TODO: Incorporate admin of msglog (see spider for partial solution)
...
HBUF *_inputBuf = NULL;						// Holds all data received
...
static char _id[] = "@(#)MMTS version " VERSION " compiled " __DATE__ " at " __TIME__;
...
void NoteLog(const char *szText)
{
	...
	if (fp) {
		...
		fprintf(fp, "%02d:%02d.%02d ", csecs / 6000, (csecs / 100) % 60, csecs % 100);
		fprintf(fp, "%s\n", szText);
		fclose(fp);
	}
...
void InitReadStream(int fd)
{
	...
	if (bDebug) {
		...
		const char *szFilename = hprintf(NULL, "/tmp/mmts.in.%02d-%02d-%02d_%02d%02d%02d.%02d.txt",
				tm->tm_mday, tm->tm_mon+1, tm->tm_year % 100,
				tm->tm_hour, tm->tm_min, tm->tm_sec,
				csecs);
...
//Log("Clause(%d): '%s'", nLen, szText);
	...
	else if (!strnicmp(chp, "like", 4)) { nOpLen=4; bWildcard=1; }	// NB. Set 'bWildcard' flag here
...
//Log("Expression: %s", szText);
	if (stristr(szText, " AND ")) {
		...
		szRover=szText;
		szAnd=(const char *)stristr(szRover, " AND ");
		...
		szResult=hprintf(szResult, "'(|");
		szRover=szText;
		szOr=(const char *)stristr(szRover, " OR ");
		for (;;) {							// Termination condition is in the following 'else'
			if (szOr) {
				chp=szOr;
				while (chp>szRover && isspace(chp[-1])) chp--;
...
{
	const char *szFrom = (const char *)stristr(szSql, " FROM ");
	const char *szWhere = (const char *)stristr(szSql, " WHERE ");
...
//Log("LdapFromSql: %s", szSql);
	if (strnicmp(szSql, "SELECT ", 7)) return "*SELECT expected at start of query";
...
//	Log("Where = '%s'", szWhere);
	...
	chp=strrchr(szFrom, '.');
	if (chp &&
			(!stricmp(chp, ".base") || !stricmp(chp, ".one") || !stricmp(chp, ".sub") || !stricmp(chp, ".children"))) {
...
// Notes that the resultant XML won't be NULL but might well be an error...
{
	...
	if (!szLdapHost) {							// Try to default to something sensible
		if (!szEnvironment || !*szEnvironment || !stricmp(szEnvironment, "live") || !strcmp(szEnvironment, "0")) {
	...
	if (fp) {									// Run the command ok so we read and XMLify the result
		...
		while (szLine=ReadLine(fp)) {
			...
			if (chp) {			// Generally assume it's "name: value"
				...
				for (cp=szLine;cp++;cp<chp) {	// Need to check 'name' is a simple alphanumeric string
					if (!isalnum(*cp)) break;
				}
	...
	if (rxResults && !rogxml_ErrorNo(rxResults)) {
		...
		if (rxTmp) {												// Step through the values looking for ours
			for (rxValue=rogxml_FindFirstChild(rxTmp);rxValue;rxValue=rogxml_FindNextSibling(rxValue)) {
				if (!stricmp(rogxml_GetAttr(rxValue, "name"), szLdapVar)) {		// Found it
...
// Changes the party ids at the given element.
{
	...
	for (i=1;i<=rogxpath_GetCount(rxp);i++) {
		...
		if (!strcmp(szRoot, "2.16.840.1.113883.2.1.3.2.4.10") ||		// NPfIT Messaging Endpoint ids
			...
			szDelete(szNewExt);
			szDelete(szFilename);
		} else if (!strcmp(szRoot, "1.2.826.0.1285.0.2.0.107")) {		// Asid needs to go here
			rxAsid=rx;
		}
	}
...
// Returns 0 if an ack was sent, non-0 otherwise.
{
	...
	time_t nRetryInterval = msg_PeriodToSeconds(msg_GetRetryInterval(msg));
...
varset *SplitCertificate(varset *v, X509 *cert)
// Pass in NULL or an existing varset along with a certificate.
// The contents of the certificate will be placed in the varset.
// A new varset is created if a NULL is passed in.
{
	if (!v) v=varset_New();
	...
	if (cert) {
		...
		for (i=0;i<nEntries;i++) {
			...
			pValue=ASN1_STRING_data(str);
			nValueLen = ASN1_STRING_length(str);
			varset_Set(v, szNameBuf, (char*)pValue);
//printf("'%s'='%s'\n", szNameBuf, (char*)pValue);
		}
	}
...
const char *FilterLink(int d, int m, int y, const char *szText, int nMax)
// Return an effectively static string that will link to a search screen with n items filtered by the string
// The display length will be limited to 'nMax' if it is non-0
// Something like "<a href=\"/status?n=100&f=Foo+Bar\">Foo Bar</a>
{
	...
	if (nMax) {
		const char *szCopy = strnappend(NULL, szText, nMax);
		szResult=hprintf(NULL, "<a href=%s>%s</a>", StatusLink(d, m, y, 0, szText), szCopy);
		szDelete(szCopy);
	} else {
		szResult=hprintf(NULL, "<a href=%s>%s</a>", StatusLink(d, m, y, 0, szText), szText);
	}
...
// Callback for qsort when sorting directory names in TableMessageLog()
{
	return (*(int*)a) - (*(int*)b);
}
...
void cb_DisplayLogLines(BIO *io, int bDeep, const char *szFilter, int day, int month, int year)
// Callback function that displays log lines for a given day, month and year
// If szFilter is non-null, only displays things that have filter in the info file (or other files if bDeep)
{
	static int donegap=0;
	const char *szDay = hprintf(NULL, "%s/%02d/%02d/%02d", szMsgLogDir, year, month, day);
	char **dirs=NULL;
	int nDir=0;
	int i;

	DIR *dDay = opendir(szDay);			// dxxx is directory handle, exxx is entry
	struct dirent *d;

	while (d = readdir(dDay)) {
		if (*d->d_name == '.') continue;

		if (nDir % 100 == 0) {
			if (nDir) {
				RENEW(dirs, char*, nDir+100);
			} else {
				dirs=NEW(char*, 100);
			}
		}
		dirs[nDir++]=strdup(d->d_name);
	}
	qsort(dirs,nDir,sizeof(*dirs),cmp_str);

	int nRow=0;
	if (nDir) {
		for (i=0;i<nDir;i++) {
			const char *szDir = hprintf(NULL, "%02d/%02d/%02d/%s", year, month, day, dirs[i]);
			const char *szType;
			...
			if (szFilter) {
				int bInclude;
				const char *szMyDir = hprintf(NULL, "%s/%s", szMsgLogDir, szDir);

				bInclude = GrepFile(szMyDir, "info", szFilter);
				if (bDeep) {
					bInclude = bInclude || GrepFile(szMyDir, "rcv.xml", szFilter);
				}
			...
			const char *szLogFile = hprintf(NULL, "%s/%s/log", szMsgLogDir, szDir);
			...
			szColour = "white";
			if (!stricmp(szProject, "Gen")) szColour="#ffddff";
			if (!stricmp(szProject, "PDS")) szColour="#ffdddd";
			if (!stricmp(szProject, "CaB")) szColour="#ddffdd";
			if (!stricmp(szProject, "eTP")) szColour="#ffffdd";

			if (!nRow) {
				if (donegap) {
					BIO_putf(io, "<tr height=\"5px\"><td colspan=14 bgcolor=\"#ffffff\"></td></tr>");
				} else {
					donegap=1;
				}
			...
			BIO_putf(io, "<td><a href=\"/getinteraction/%s\">", szDir);
			BIO_putf(io, "%.2s%.2s%.2s%s", szDir, szDir+3, szDir+6, szDir+9);
			BIO_putf(io, "</a>");

			BIO_putf(io, "<td nowrap=1>%s</td>", FilterLink(day, month, year, szEnv, 0));
			BIO_putf(io, "<td nowrap=1>%s</td>", FilterLink(day, month, year, szFrom, 0));
			BIO_putf(io, "<td nowrap=1>%s</td>", FilterLink(day, month, year, szTo, 0));
			BIO_putf(io, "<td nowrap=1>%s</td>", FilterLink(day, month, year, szNhs, 0));
			if (*szUuid == '-') {
				BIO_putf(io, "<td>-</td>");
			} else {
				BIO_putf(io, "<td class='TD_MSGID'>%s</td>", FilterLink(day, month, year, szUuid, 8));
			}
			if (*szConv == '-') {
				BIO_putf(io, "<td>-</td>");
			} else {
				BIO_putf(io, "<td class='TD_MSGID'>%s</td>", FilterLink(day, month, year, szConv, 8));
			}
			BIO_putf(io, "<td class='TD_%s' nowrap=1>%s</td>", szProject, FilterLink(day, month, year, szProject, 0));
			if (szZInfo) {
				BIO_putf(io, "<td nowrap=1 colspan=2>%s</td>", FilterLink(day, month, year, szZInfo, 0));
			} else {
				BIO_putf(io, "<td nowrap=1>%s</td>", FilterLink(day, month, year, szType, 0));
				BIO_putf(io, "<td nowrap=1>%s</td>", FilterLink(day, month, year, szInteractionName, 0));
			}
			BIO_putf(io, "<td>%s</td>", FilterLink(day, month, year, szIP, 0));
...
void DisplayLinks(BIO *io, int m, int y, int bDeep, const char *szFilter)
{
	...
	if (!y) {
		...
		if (y % 100 == tm->tm_year % 100)
			m = tm->tm_mon+1;
		else
			m = 12;
	}

	BIO_putf(io, "%s 20%02d: ", MonthNames[m], y);
	const char *szMonth = hprintf(NULL, "%s/%02d/%02d", szMsgLogDir, y, m);
	...
	if (IsDir(szMonth)) {
		...
		for (d=1;d<=31;d++) {
			const char *szDay = hprintf(NULL, "%s/%02d", szMonth, d);
			if (IsDir(szDay)) {
				int nCount = DirCount(szDay);

				if (nCount) {
					if (doneone) BIO_putf(io, ", ");
					doneone=1;
					BIO_putf(io, " <a href=%s>%d%s</a> (%d)", StatusLink(d, m, y, bDeep, szFilter), d, th(d), nCount);
				}
			}
			szDelete(szDay);
		}
	...
	y = tm->tm_year%100;
	while (gotone) {					// Always does at least one, which is convenient
		gotone=0;

		const char *szYear = hprintf(NULL, "%s/%02d", szMsgLogDir, y);
		for (m=1;m<=12;m++) {
			const char *szMonth = hprintf(NULL, "%s/%02d", szYear, m);
			if (IsDir(szMonth)) {
				if (!gotone)
					BIO_putf(io, "<a href=%s>20%02d</a>", StatusLink(0,0,y, bDeep, szFilter), y);
				BIO_putf(io, " <a href=%s>%.3s</a>", StatusLink(0, m, y, bDeep, szFilter), szMon+(m-1)*3);
				gotone=1;
			}
			szDelete(szMonth);
		}
...
void ForEachLogMonth(void (*cb)(BIO *io, int bDeep, const char *szFiler, int d, int m, int y), BIO *io, int bDeep, const char *szFilter, int day, int month, int year)
// Calls the given function with every day, month and year that we have a log entry for.
// This is always called in reverse order of date.
{
	int years[100]; int nyears;
	int months[100]; int nmonths;
	int days[100]; int ndays;
	int yy, mm, dd;

//BIO_printf(io, "<hr>ForEachLogMonth(%p, %p, %d, %d, %d)<hr>", cb, io, day, month, year);
	// Comments assume a date of 14-01-2013
	DIR *dMsgLog = opendir(szMsgLogDir);			// dxxx is directory handle, exxx is entry
	struct dirent *eMsgLog;
	nyears=0;
	while (eMsgLog = readdir(dMsgLog)) {
		const char *y = eMsgLog->d_name;										// 13

//BIO_printf(io, "(%s)", y);
		if (strlen(y) == 2 && isdigit(y[0]) && isdigit(y[1])) {
//BIO_printf(io, "[year = %s]", y);
			years[nyears++]=atoi(y);
		}
	}
	closedir(dMsgLog);
	qsort(years, nyears, sizeof(int*), cmp_int);

	for (yy=nyears-1;yy>=0;yy--) {
		const char *szYear = hprintf(NULL, "%s/%02d", szMsgLogDir, years[yy]);		// msglog/13
		DIR *dYear = opendir(szYear);
		struct dirent *eYear;
		nmonths=0;

		while (eYear = readdir(dYear)) {
			const char *m = eYear->d_name;

			if (strlen(m) == 2 && isdigit(m[0]) && isdigit(m[1])) {
				months[nmonths++]=atoi(m);
//BIO_printf(io, "[month = %s/%d]", m, years[yy]);
			}
		}
		closedir(dYear);
		qsort(months, nmonths, sizeof(int*), cmp_int);

		for (mm=nmonths-1;mm>=0;mm--) {
			const char *szMonth = hprintf(NULL, "%s/%02d", szYear, months[mm]);		// msglog/13/01
			DIR *dMonth = opendir(szMonth);
			struct dirent *eMonth;
			ndays=0;

			while (eMonth = readdir(dMonth)) {
				const char *d = eMonth->d_name;

				if (strlen(d) == 2 && isdigit(d[0]) && isdigit(d[1])) {
//BIO_printf(io, "[day = %s/%d/%d]", d, months[mm], years[yy]);
					days[ndays++]=atoi(d);
				}
			}
			closedir(dMonth);
			qsort(days, ndays, sizeof(int*), cmp_int);

			for (dd=ndays-1;dd>=0;dd--) {
				if ((!year || year==years[yy]) &&
						(!month || month==months[mm]) &&
						(!day || day==days[dd])) {
					(*cb)(io, bDeep, szFilter, days[dd],months[mm],years[yy]);
				}
			}
		}
	}
...
void TableMessageLog(BIO *io, int bDeep, const char *szFilter, int day, int month, int year)
{
//	BIO_printf(io, "TableMessageLog(%p,%d,%d,\"%s\",%d,%d)<br>", io, bDeep, szFilter, day, month, year);
	if (!year && !month && !day) {
		time_t now=time(NULL);
		struct tm *tm=gmtime(&now);
		year=tm->tm_year % 100;
	...
	DisplayLinks(io, month, year, bDeep, szFilter);
	MyBIO_puts(io, "<style>"
	...
	ForEachLogMonth(cb_DisplayLogLines, io, bDeep, szFilter, day, month, year);
...
//		200		Ok, etc...
{
	...
	int i;
	rogxml *rx;
	int nContentLength = 0;				// Maybe we need this header...
...
//Log("Soap action = '%s'\n", szSoapAction);
	if (!szSoapAction) {
		if (!stricmp(msg_GetInteractionId(msg), "PAFToolQuery")) { // Gazetteer kludge...
...
//Log("Adding attachment %d", i);
		bBase64 = !stristr(szContentType, "/xml");		// We'll base64 it if it's not XML for now
...
const char *HttpHeader(int code, const char *text)
{
	return hprintf(NULL, "HTTP/1.1 %d %s\r\n", code, text ? text : HttpResponseText(code));
}
...
//					1...	Error code and pMime will point to text (heap-based)
{
	...
	const char *szHttpCode = NULL;					// Text for HTTP code
	...
	if (*szText == '<') {
		...
		if (szAttrHttpText) szHttpCode = strdup(szAttrHttpText);
...
//				szReason = rogxml_GetAttr(rxError, "message");
	...
	if (pResponse) {
		*pResponse = szResponse ? szResponse
							   : HttpHeader(nHttpCode, szHttpCode);
	}
	szDelete(szHttpCode);
...
// Returns a string on the heap comprising the entire message including HTTP header
{
	...
	snprintf(szContentLength, sizeof(szContentLength), "%d", strlen(szBody));
	mime_SetHeader(mime, "Content-Length", szContentLength);

	mime_AddBodyPart(mime, XML_MIME_TYPE, -1, szBody);
	if (nHttpCode) {
		const char *szHead = HttpHeader(nHttpCode, NULL);
		const char *szBody = mime_RenderHeap(mime);
...
//	rogxml_AddTextChild(rxMsgData, "eb:MessageId", WrapperGetMessageId(rxHL7));
	...
	if (!stricmp(contract_GetDuplicateElimination(c), "always")) {
...
//	rogxml_AddTextChild(rxMsgData, "eb:TimeToLive", "0");		// Optional in P1R1?
	...
	if (stricmp(contract_GetSyncReplyMode(c), "None")) {			// Anything but 'None'
...
//	if (bAckRequested) {						// Needs slightly kludgey, post-soapy way to link in ack request
	if (stricmp(contract_GetAckRequested(c), "Never")) {			// Anything but 'Never'
...
rogxml *MakeItkAcknowledgement(const char *szTo, const char *szId)
{
	...
	rogxml *rxBody = rogxml_AddChild(rx, "s:Body");
...
//	rogxml_AddTextChild(rxMsgData, "eb:TimeToLive", "0");		// Optional in P1R1?
	...
	rxAck = rogxml_NewElement(NULL, "eb:Acknowledgment");
	...
	rogxml_AddTextChild(rxAck, "eb:RefToMessageId", WrapperGetMessageId(rxHL7));
...
int AddSoap(MSG *msg)
{
	...
	rxSOAP = rogxml_NewElement(NULL, "soap:Envelope");
	rogxml_AddNamespace(rxSOAP, "soap", "http://schemas.xmlsoap.org/soap/envelope/");
	rxHeader=rogxml_AddChild(rxSOAP, "soap:Header");
	rxBody=rogxml_AddChild(rxSOAP, "soap:Body");
}
...
{
//#define MMTS_BGCOL1	"#f57023"			// Main MMTS title background
//#define MMTS_BGCOL2	"#2b509a"			// details of connection
//#define MMTS_BGCOL3	"#b8541b"			// Links etc. under main title
...
// $mtorange="#f57023"; $mtblue="#2b509a";
	...
	BIO_putf(io, 
		...
		"<tr><td rowspan=2 colspan=2 align=center><font face=\"calibri,verdana,arial\" size=5 color=" MMTS_FGCOL1 ">MMTS v" VERSION " - The Microtest Message Transfer System</font>"
...
// szEnvironment will be set to a string on exit from here or a fatal error will have occurred
{
	...
	if (!szEnv) {												// Nothing for this address or port or even a default
		...
		if (nCount > 1) {
			Fatal("Multiple environments available, but no default");
		}
...
void SendHttpHeader(BIO *io, int nCode, const char *szContent)
{
...
const char *FileContentType(const char *szFilename)
{
	...
	if (ext) {
		...
		if (!map) {
			...
			ssmap_Add(map, "doc", "application/ms-word");
...
int DeliverFile(BIO *io, const char *szFilename, const char *szContentType)
{
	...
	if (!fp) {
		MyBIO_puts(io, HttpHeader(403, NULL));		// Forbidden
		return 0;
	}
	...
	if (!szContentType) szContentType = szContentType = "text/plain";

	MyBIO_puts(io, HttpHeader(200, NULL));		// Forbidden
	MyBIO_puts(io, "Server: Microtest Message Transport Server\r\n");
	BIO_putf(io, "Content-Type: %s\r\n", szContentType);
...
int DealWithGET(BIO *io, const char *szURI)
{
	...
	if (!strnicmp(szURI, "/spider", 7))			return Proxy(io, "127.0.0.1", 4511, "GET", "/spider", szURI+7);
	if (!strnicmp(szURI, "/local", 6))			return Proxy(io, "127.0.0.1", 80, "GET", "/local", szURI+6);
	if (!strnicmp(szURI, "/gpesmmts", 9))		return Proxy(io, "gpes", 10024, "GET", "/gpesmmts", szURI+9);
	if (!strnicmp(szURI, "/gpesspider", 11))	return Proxy(io, "gpes", 4511, "GET", "/gpesspider", szURI+11);
	if (!strnicmp(szURI, "/gpes", 5))			return Proxy(io, "gpes", 80, "GET", "/gpes", szURI+5);
	...
	if (chp) {
		...
		for (;;) {
			...
			if (pEq) {
				*pEq='\0';
				if (nParams) {
					szNames=RENEW(szNames, const char *, nParams+1);
					szValues=RENEW(szValues, const char *, nParams+1);
				} else {
...
//		EchoCertificate(io, certPeer);
		...
		szDelete(szIconFile);
	} else if (!strnicmp(szAddr, "/file/", 6) || !stricmp(szAddr, "/file")) {
		...
		if (err) {
			MyBIO_puts(io, HttpHeader(404, NULL));		// Not found
			return 0;
		}
		...
		if (st.st_mode & S_IFDIR) {
			...
			MyBIO_puts(io, HttpHeader(403, NULL));		// Forbidden
			MyBIO_puts(io, "Content-Length: 20\r\n");
			MyBIO_puts(io, "\r\n");
			MyBIO_puts(io, "You can't touch this");
			return 0;
		}
		...
		if (szParam && !stricmp(szParam, "env")) {
...
//BIO_putf(io, "Params='%s', Addr='%s', szParam='%s', szValue='%s'<br>", szParams, szAddr, szParam, szValue);
		...
		TableMessageLog(io, bDeep, szFilter, d, m, y);
		...
		SendHttpHeader(io, 200, szContentType);
...
//			0	Failed to deliver - handler not found?
{
	...
	if (nWaiting == 1) {								// Old waiting method with .waiting files
		...
		const char *szCopyId = strdup(szRefId);
		strupr(szCopyId);
		...
		const char *szCopyConvId = strdup(szConvId);
		strupr(szCopyConvId);
...
static int xverify_callback_snd(int preverify_ok, X509_STORE_CTX *ctx)
{
	char    buf[256];
	X509   *err_cert;
	int     err, depth;
	...
	err = X509_STORE_CTX_get_error(ctx);
	depth = X509_STORE_CTX_get_error_depth(ctx);
Log("verify_callback_snd(%d, %p) - err_cert=%p, err=%d, depth=%d", preverify_ok, ctx, err_cert, err, depth);
...
//Log("ssl = %p, mydata = %p", ssl, mydata);
	...
	if (!preverify_ok) {
		Log("verify error:num=%d:%s:depth=%d:%s", err,
				X509_verify_cert_error_string(err), depth, buf);
	}
	else if (mydata->verbose_mode)
	{
		Log("depth=%d:%s", depth, buf);
	}
...
// docs/abc/def.sh, docs/abc, docs/abc.pl, docs/abc.php, docs/abc.sh
{
	...
	int bMultipart = 0;								// 1 if the form is multipart/form-data

	const char *szContentType = mime_GetContentType(mime);
	bMultipart = szContentType && !strnicmp(szContentType, "multipart/", 10);
	...
	if (szExecutable) {
		...
		MyBIO_puts(io, "Server: Microtest\r\n");
		MyBIO_puts(io, "\r\n");
		MyBIO_puts(io, "The form handler was not found - sorry.");
	}
...
// The header returned by the remote server will be placed in *pHeader
{
	...
	if (!io) {
		if (pHeader) *pHeader = strdup(HttpHeader(503, NULL));
...
//Log("Rel = '%s'", szRelPrefix);
	const char *href = (const char *)stristr(szBody, szReference);
	while (href) {
...
//Log("AFTER: %s", szBody);
		...
		href=(const char *)stristr(href+1, szReference);
	}
...
// http output.
{
	...
	if (http_GetLastStatusCode() == 301) {			// We're being redirected
		...
		if (szLocation) {
			...
			if (nRedirPort == 0) nRedirPort=!stricmp(szRedirProtocol, "https") ? 443 : 80;	// It wasn't specified
//Log("Split %s to (%s,%s,%d,%s)", szLocation, szRedirProtocol, szRedirAddr, nRedirPort, szRedirUri);
			if (!stricmp(szRedirProtocol, "http") && strcmp(szRedirAddr, szAddr)) {
...
//Log("Body = '%s'", szBody);
	...
	if (szContentType && !stricmp(szContentType, "text/html")) {		// Need to translate hrefs...
...
{
	static const char *szzUnderstood = "MessageID\0Action\0To\0From\0Security\0FaultTo\0ReplyTo\0MessageHeader\0AckRequested\0SyncReply\0";
	rogxml *rx;
...
//	int bAuth;
	...
	int i;
	rogxml *rx;
	contract_t *contract;						// Contract for received message

	Log("HTTP POST %s", szURI);

	if (!strnicmp(szURI, "/spider", 7))			return Proxy(io, "127.0.0.1", 4511, "POST", "/spider", szURI+7);
	if (!strnicmp(szURI, "/local", 6))			return Proxy(io, "127.0.0.1", 80, "POST", "/local", szURI+6);
	if (!strnicmp(szURI, "/gpesspider", 11))	return Proxy(io, "gpes", 4511, "POST", "/gpesspider", szURI+11);
	if (!strnicmp(szURI, "/gpes", 5))			return Proxy(io, "gpes", 80, "POST", "/gpes", szURI+5);
...
//Log("Content type = '%s'", szContentType);
	if (!stricmp(szContentType, "application/x-www-form-urlencoded")
		|| !stricmp(szContentType, "multipart/form-data")) {
...
// Mantis 15333 - I'm unsure whether this approach is correct on incoming NASP, but I can't really see why not
		...
		if (delivered) {
			if (szReply) {
				rxReply = rogxml_FromText(szReply);
...
//				int err = MakeHttpFromSoapText(&szResponse, &szMime, szReply, "urn:nhs-itk:services:201005:SendInfrastructureAck-v1-0", szFrom, szTo);
				...
				if (!err) {
					...
					MyBIO_puts(io, HttpHeader(503, NULL));			// Service Unavailable
				...
				MyBIO_puts(io, HttpHeader(200, NULL));				// OK
			}
		} else {
			MyBIO_puts(io, HttpHeader(405, NULL));					// Method not allowed
		}
...
static void xServeRequest(BIO *io)
// Talk to the Client that's calling us
{
	char buf[1024];
	int nGot;					// Bytes got from a read
...
// If the timeout expires, returns NULL otherwise waits forever.
{
	...
	if (nTimeout) {							// Arrange that we get a wakeup call if necessary
		...
		if (setjmp(jmpbuf_alarm)) {
			...
			int nPrevious = alarm(nTimeout);
//			Log("Alarm set for %d (was %d)", nTimeout, nPrevious);
		}
	}
...
// Returns 1 if we've fallen back to the old '.waiting' method
{
	...
	if (!stricmp(msg_GetInteractionId(msg), "Acknowledgment") || !stricmp(msg_GetInteractionId(msg), "Acknowledgement"))
		return 0;
...
// We'll use a counter over the number of waits here to avoid any problem with radical changes in the time/date.
{
	...
	const char *szCopyId=strdup(szId);
	strupr(szCopyId);
	const char *szWaiting = hprintf(NULL, "%s/%s.waiting", szWaitingDir, szCopyId);
...
//TODO: XYZZY Need to do soemthing what the XML we've just received...!
		...
		nPorts = select(fd+1, NULL, NULL, &fdseterr, &timeout);
//		Log("fd=%d: nPorts = %d, errno=%d, err=%d", fd, nPorts, errno, FD_ISSET(fd, &fdseterr));
//		Log("Write returns %d, errno=%d", write(fd, "", 0), errno);
//		Log("fcntl = %d", fcntl(fd, F_GETFL));
		if (!fd) {
...
//	msg_ExtractMessageInfo(msg);				// Do this a second time now that it's wrapped
...
// determined from these contract properties.
					contract_t *c = msg_GetContract(msg);
					if( stricmp(contract_GetSyncReplyMode(c),"none") && stricmp(contract_GetAckRequested(c),"never") ) {
...
szCheck="A";
//Log("Certificate check = '%s'", szCheck);
...
//	int nErr = SSL_get_error(ssl, nGot);
	...
	err = SSL_get_verify_result(ssl);
	peer=SSL_get_peer_certificate(ssl);
//Log("io=%p, ssl_bio=%p, err = %d, peer = %p", io, ssl_bio, err, peer);
...
// Currently only handles requests for GUIDs (a random one is generated and sent back)
{
	...
	if (!stricmp(szRequest, "uuid") || !stricmp(szRequest, "guid")) {
...
// Returns the filename if found or NULL.  The filename will have to be Delete'd
{
	...
	if (dir) {
		while (d=readdir(dir)) {
			...
			if (nNameLen < 4 || stricmp(d->d_name+nNameLen-4, ".xml"))
		...
		else if( strcmp( name, "stdin" ) == 0 )
		{
			...
			for( node = rogxml_FindFirstChild(rx); node != NULL; node = rogxml_FindNextSibling(node) )
			{
				...
				if( buf != NULL ) {
					...
					stdin_buf     = (char*)realloc(stdin_buf,buf_len);
...
//
{
	...
	if (!strnicmp(szText, "like") && !isalnum(szText[4])) {	// Special case for 'like'
...
// Performs a general SDS lookup and returns the result
{
	...
	if (szFunction) {
		if (!stricmp(szFunction, "PartykeyFromAsid")) {
		...
		} else if (!stricmp(szFunction, "PartykeyFromNacs")) {
		...
		} else if (!stricmp(szFunction, "NacsFromPartykey")) {
...
int DirectMessage(MSG *msg)
// Takes the message accepted from the source and decides what to do with it next.
{
}
...
//		Note("C|%s", msg_GetConversationId(msg));
	...
	if (rxBody) {
		...
		if (!szAddress || !nPort || !szUri) {
			SendError(fd, 404, 1, "Could not decode endppoint '%s'", szEndpoint);
			return 4;
		}
...
//	Log("Transforming to internal message");
	...
	if (nErr = SendResponse(fd, msg)) {
		Fatal("Error %d processng local message");
	}
...
//TODO: Only want one of the following two!
		...
		case 1:									// 1 - HTTP error returned from the far end
			Log("Non-OK response received (%d), returning it to the app", msg_GetHttpCode(msgResponse));
		...
		case 2:									// 2 - Message returned with error in it (can't really happen)
			...
			Log("MMTS Error %d - %s", msg_GetErrorNo(msgResponse), msg_GetErrorText(msgResponse));
			msgReply = msgResponse;				// Message for app is in the response hopefully
			break;
		...
		if (bNeedRetry) {
			if (msg_GetSends(msg) >= msg_GetMaxRetries(msg)) {	// We've already tried enough times
				...
				if (!fork()) {								// Create us a new process, which is this block
...
//Log("%s is due...", szName);
					...
					if (ProcessAlive(pid)) {
						...
						if (pid=fork()) {
							int nChildren = child_Add(pid, 0, NULL, msg_GetMessageId(msg));
...
// comparisons that depend on it.
{
	...
	int nTimeDiff;										// Number of seconds by which time has wandered
	int i;
	...
	nTimeDiff = now-nTimeTrack;							// Number of seconds by which time has wandered
	nCounter++;
	...
	// Quits the daemon nice and tidily
{
	...
	if (bIsDaemon) {
		...
		if (_inputBuf) {							// We have an input buffer we want to write out
			if (!_bNoteInhibit) {
				const char *szFilename=hprintf(NULL, "%s/rawin.txt", NoteDir());
				FILE *fp = fopen(szFilename, "w");
				if (fp) {
					int len = hbuf_GetLength(_inputBuf);
					const char *buf = hbuf_ReleaseBuffer(_inputBuf);
					fwrite(buf, len, 1, fp);
					szDelete(buf);
					fclose(fp);
				}

				Note("M|%s|%s", "rawin.txt", "Raw incoming data (on the wire)");
				szDelete(szFilename);
			}
...
//		}
	...
	for (;;) {
		...
		timeout.tv_sec=0;						// Seconds
		timeout.tv_usec=500000;					// Microseconds	- Must be 1/2 seconds as Idle() depends on that freq.
		...
		if (nSrc == -1) {						// No network port ready, we'll check other places
			szDropped=CheckDropDir();
			if (szDropped) {
				nSrc=SRC_DROPPED;
			} else {							// Other possible polling checks in here
			}
		}
		...
		MessageId();							// Trigger a new message ID
		if (pid=fork()) {						// Master
			...
			ipc_Id(getpid());					// Ensure we have a message queue

			Log("MESSAGE: MMTS version " VERSION ", compiled " __DATE__ " at " __TIME__);
...
//			case 'P': nListenPort_ext = atoi(optarg);		break;	// Set external port (default is 443)
	...
	s3 *s = s3_Open(0, NULL);
	printf("s = %p\n", s);
	int err=s3_Do(s, "CREATE TABLE IF NOT EXISTS dupcheck (uuid CHAR(36), dt DATETIME DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (uuid))");
	if (err) printf("Error %d: %s\n", s3_ErrorNo(s), s3_ErrorStr(s));
	err=s3_Do(s, "INSERT INTO dupcheck VALUES ('00000000-0000-0000-0000-000000000000',NULL)");
	if (err) printf("Error %d: %s\n", s3_ErrorNo(s), s3_ErrorStr(s));
	err=s3_Do(s, "INSERT INTO dupcheck VALUES ('00000000-0000-0000-0000-000000000001',NULL)");
	if (err) printf("Error %d: %s\n", s3_ErrorNo(s), s3_ErrorStr(s));
	err=s3_Do(s, "INSERT INTO dupcheck VALUES ('00000000-0000-0000-0000-000000000002',NULL)");
	if (err) printf("Error %d: %s\n", s3_ErrorNo(s), s3_ErrorStr(s));
	err=s3_Do(s, "INSERT INTO dupcheck VALUES ('00000000-0000-0000-0000-000000000003',NULL)");
	if (err) printf("Error %d: %s\n", s3_ErrorNo(s), s3_ErrorStr(s));

	const char **row;
	s3it *it=s3it_New(s, "SELECT uuid FROM dupcheck");
	if (it) {
		while (row=s3it_Next(it)) {
			printf("GUID = '%s'\n", row[0]);
		}
	} else {
		printf("Error %d: %s\n", s3_ErrorNo(s), s3_ErrorStr(s));
