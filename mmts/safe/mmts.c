...
// Problem at Park Hill - /usr/mt/mmts/msglog/14/04/05/aaabn/log
...
Log("err was about to be returned as %d", err);
err=0;			// TODO: This allows any certificate to be accepted - remove it once we fix the Bissoe Rd problem!
...
//Log("HTTP Request: %s : %s : %s", szMethod, szURI, szVersion);
	...
	if (!strcasecmp(szMethod, "GET")) {		n=DealWithGET(io, szURI); }
	else if (!strcasecmp(szMethod, "PUT")) {	n=DealWithPUT(io, szURI); }
	else if (!strcasecmp(szMethod, "POST")) {	n=DealWithPOST(io, szURI); }
	else {
...
//		}
	...
	for (;;) {
		...
		if ((pid=fork())) {						// Master
			...
			int nChildren = child_Add(pid, _szSenderIp, NULL);
			Log("Child #%d (%d) processing connection from %s", nChildren, pid, _szSenderIp);
			if (fd != -1)
