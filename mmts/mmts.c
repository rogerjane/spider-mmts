...
// 30-03-17 RJ 2.51 Fixed incorrect tmp directory (used /mt instead of /usr/mt!)
// 05-04-17 RJ 2.52 Now reads api_redirect.map sequentially
// 18-04-17 RJ 2.53 A couple mods to make Ben's like easier for witness testing of GP Connect (look for 2.53)
// 20-04-17 RJ 2.54 Send responses as chunked if they're over 10k
// 25-04-17 RJ 2.55 Pass X-MT-TLS etc. over to invoked WAMP procedure
// 10-05-17 RJ 2.56 Directly returns msglog files, rather than use perl 'message' script
// 22-05-17 RJ 2.57 Allows proxying by simply placing the destination address as the URL
// 24-05-17 RJ 2.58 Understands 'CONNECT' HTTP verb
// 11-06-17 RJ 2.59 Include X-MMTSLOGDIR in outgoing headers
...
const char *_szTlsPeerIp = NULL;			// Either _szSenderIp or _szReceiverIp - used in verify_callback_rcv()
...
NetworkPort_t *_incoming_networkport = NULL;	// The information on the incoming connection
const char *_tls_cname = NULL;				// Common name of peer
const char *_tls_version = NULL;			// TLSv1, TLSv1.1 or TLSv1.2 for example
const char *_tls_cipher = NULL;				// Cipher used in text form
bool _tls_nhs = false;						// True if we recognise the NHS root certificate
...
char mmts_sccs_id[] = "@(#)MMTS version " VERSION " (" OS ") compiled " __DATE__ " at " __TIME__;
...
const char *NiceNhs(const char *nhs)
{
	...
	if (nhs && strlen(nhs) == 10) {		// 10 chars so might be an NHS
		const char *chp = nhs;
		...
		if (!*chp) {					// All digits - yay
			result = hprintf(NULL, "%.3s %.3s %s", nhs, nhs+3, nhs+6);
		}
	}

	return result ? result : nhs;
}
...
void NoteMessage(const char *szMessage, int nLen, const char *szTag, const char *szExt, const char *szDescr)
{
	...
	bool messageOnHeap = false;					// True if we manipulate szMessage to be on the heap

	if (!strcmp(szExt, "*")) {					// szExt is * - we're being asked to guess the body type!
		bool done = false;

		JSON *json = json_ParseText(szMessage);
		const char *jsonError = json_Error(json);
		if (!jsonError) {							// Looks like jSON
			szExt = "json";
			szMessage = json_Render(json, true);
			nLen = -1;
			done = true;
		}
...
// SSL errors so we can return more information
{
	...
	Log("SSL Problem so quitting: %s", buf);		// An error essentially, but we don't care enough to make everything pink
...
// NB. Returns szMsg IFF no translation has taken place.
{
	...
	char *szOriginal = hprintf(NULL, "/usr/mt/tmp/mmts_original_%d", getpid());

	fp=fopen(szOriginal, "w");					// Strangely can't open temp file - disk full?  Someone deleted the dir???
	if (!fp) {
		Log("Cannot create temporary file for translation at '%s'", szOriginal);
		szDelete(szOriginal);
		return szMsg;
	}
	...
	char *szTranslated = hprintf(NULL, "/usr/mt/tmp/mmts_translated_%d", getpid());

	szCommand = hprintf(NULL, "%s %s %s", szTranslator, szOriginal, szTranslated);

	int err = system(szCommand);				// Really we don't care if this succeeds or fails...
	if (err)
		Log("Error %d executing: %s", err, szCommand);
...
//	meth=SSLv23_method();							// Create our context
	meth=TLS_method();										// Create our context
	...
	// Display the content of the certificate
{
	if (cert) {
		...
		for (i=0;i<nEntries;i++) {
			...
			pValue=ASN1_STRING_get0_data(str);
...
const char *note_FindEntry(note_t *n, char c, const char *def = NULL)
{
...
const char *FilterDayLink(int day, int month, int year, const char *szText, const char *display = NULL)
// Return an effectively static string that will link to a search screen with items filtered by the string
{
	...
	if (!display) display = szText;

	if (year < 99) year+=2000;
	if (szText && *szText) {
		const char *szEncodedFilter = UriEncode(szText);
		szResult=hprintf(NULL, "<a href=\"/status/%04d/%02d/%02d?f=%s\">%s</a>",
				year, month, day, szEncodedFilter, display);
		szDelete(szEncodedFilter);
	} else {
		szResult=hprintf(NULL, "<a href=\"/status/%04d/%02d/%02d\">%s</a>", year, month, day, display ? display : "");
	}
...
//BIO_putf(io, "Looking in %s/%s for bollocks", dir, "log");
	...
	if (szZInfo && !strncmp(szZInfo, "SDS Query", 9)) { szProject="SDS"; }		// Don't like this

	if (!strcmp(szProject, "SDS")) return;							// 2.53: Don't display SDS requests
	...
	BIO_putf(io, "<td nowrap=1>%s</td>", FilterDayLink(day, month, year, szNhs, NiceNhs(szNhs)));
	if (*szUuid == '-') {
		BIO_putf(io, "<td>-</td>");
	} else {
...
bool SplitEndpoint(const char *szEndpoint, const char **pszProtocol, const char **pszAddress, int *pnPort, const char **pszURI, int bAllowDefault)
...
// Returns	true	All OK
//			false	Duff in some way.  procotol, address and URI will be NULL, port will be 0
// NB. In fact, currently, there is no condition where it returns false...
{
...
const char *MakeHttpMessage(MSG *msg, int bAck, int nCode)
// nCode is the response code:
//		0		This is a POST
//		200		Ok, etc...
{
...
//Log("Rendered the message");
	...
	if (nCode) {
		szResult=strdup(http_StatusLine(nCode));
	} else {
...
void SaveAsJwt(char *text)
{
	char *chp;
	const char *jwt = NULL;
	for (chp = strtok(text, "."); chp; chp = strtok(NULL, ".")) {
		int len;
		char *b64 = (char*)mime_Base64Dec(&len, chp);
		if (b64) {
			RENEW(b64, char, len+1);
			b64[len]='\0';
			if (jwt) jwt = hprintf(jwt, "\n----------------------------------------\n");
			JSON *json = json_ParseText(b64);
			const char *err = json_Error(json);
			if (err) {
				jwt = hprintf(jwt, "JSON Error: %s\n%.*s", err, len, b64);
			} else {
				jwt = hprintf(jwt, "%s", json_Render(json, true));
			}

			// Get requesting_organisation/identifier/value - the source organisation's identifier for the 'From' entry
			JSON *requestingOrganisation = json_ObjectElementCalled(json, "requesting_organization");
			if (requestingOrganisation) {
				JSON *identifier = json_ObjectElementCalled(requestingOrganisation, "identifier");
				if (json_IsArray(identifier)) {
					identifier = json_ArrayAt(identifier, 0);
					Log("Picked identifier from array(0) = %p", identifier);
				}

				if (identifier) {
					const char *value = json_ObjectStringzCalled(identifier, "value");
			...
			// Get requested_record/identifier/value - The NHS number if it's a patient - store as NHS number
			JSON *requestedRecord = json_ObjectElementCalled(json, "requested_record");
			if (requestedRecord) {
				JSON *identifier = json_ObjectElementCalled(requestedRecord, "identifier");
				if (json_IsArray(identifier)) {
					identifier = json_ArrayAt(identifier, 0);
					Log("Picked identifier from array(0) = %p", identifier);
				}
			...
			jwt = hprintf(jwt, "Could not B64 decode:\n%s\n", chp);
		}
	}

	NoteMessage(jwt ? jwt : "Nothing decoded", -1, "jwt", "txt", "JWT received");
	szDelete(jwt);

}

SSMAP *SetIncomingHeaderFromMime(MIME *mime)
// Sets the _incomingHeader information from the MIME given
// Also:
//	* saves the JWT from the Authorization header
//	* Records the SSP-InteractionId (if there is one) as the interaction ID in the info file
{
	if (!mime) return NULL;
	...
	for (name = szzHeaders; *name; name = szz_Next(name)) {
		...
		char *value = (char*)mime_GetFullHeaderValue(mime, name);
		if (value) {
//Log("Setting header %s: %s", name, value);
			if (!stricmp(name, "Authorization") && !strnicmp(value, "Bearer ", 7))
				SaveAsJwt(value+7);
			if (!stricmp(name, "SSP-InteractionId")) {
				const char *type = value;
				if (!strnicmp(type, "urn:nhs:names:services:", 23))
					type += 23;
				NoteType("Y", type);
			}
...
// Chunking...
// If you want to send a 'chunked' message there are two ways of doing it...
// If the content is already in one block, use SendHttp() on its own:
//
//   SendHttp(io, 200, {headers}, len, content, 10240);
//
// If the content is not available yet then do this...
//
//   SendHttp(io, 200, {headers}, 0, NULL, -1)
//   SendHttpChunk(io, len, content);
//   SendHttpChunk(io, len, content);
//   SendHttpChunk(io, len, content);
//   SendHttpChunk(io, 0, NULL);
//
// Note that the only way to send compressed content is to use the first method where the received header allowed gzip
//

void SendHttpChunk(BIO *io, int len, const char *content)
// Sends a chunk of data.
// This only makes sense if 'SendHttp' has been called already with chunkSize=-1
// This function should then be called multiple times with the next chunk to send, ending with a call
// with len=0.
// As usual, len=-1 translates to strlen(content)
{
Log("SendHttpChunk(%p, %d, %p)", io, len, content);
	if (io) {
		if (!content) content="";
		...
		if (len > 0) {
			BIO_putf(io, "%x\r\n", len);
			MyBIO_write(io, content, len);
			MyBIO_puts(io, "\r\n");
		} else {
			MyBIO_puts(io, "0\r\n\r\n");			// Note that, if we can think of a reason, extra headers can go here
		}
	}
}

void SendHttp(BIO *io, int code, SSMAP *header = NULL, int len = -1, const char *content = NULL, int chunkSize = 0)
// NB. header should have been created with NewHeaderMap() so that it has the 'ignore case' setting.
// io			- The stream we'll send on
// header		- An SSMAP holding all the header lines
// len			- The length of the content (or -1 for strlen(content) or -2 for none)
// content		- A pointer to content (can be NULL to not include any content)
// chunkSize	- > 0 indicates chunked transfer-encoded in that size chunks, -1 puts chunking in the header but doesn't do content
//
...
// Call with chunkSize = -1 to put "Transfer-encoding: chunked" in the header, but sent no content (content should be NULL)
// Call with content=NULL if you want to handle sending the content yourself afterwards
//
// Note that we go through a little pain in here to ensure that the entire header is sent as a single
// block so that braindead Windows socket handlers don't trip over possible fragmented IP packets.
{
Log("SendHttp(%p, %d, %p, %d, %p, %d)", io, code, header, len, content, chunkSize);
	if (!io) return;
	...
	const char *headerBlock = NULL;		// We'll collect the data up here and send it in one go

	const char *firstLine = HttpHeader(code);					// Do the "HTTP/1.1 code message" line
	headerBlock = hprintf(headerBlock, "%s", firstLine);
	...
	// Next line containts a constant, which is the minimum size at which we'll gzip a response
	// The logic is that the network MTU is going to be 1500 bytes so there's no point in compressing
	// the content if it (and the header) is going to be below this threshold
	if (content && len > 1300 && !strcmp(InputAcceptEncoding(), "gzip")) {					// Compressing
		z_stream strm;
		strm.zalloc = Z_NULL;
	    strm.zfree = Z_NULL;
	    strm.opaque = Z_NULL;
		...
		do {
			deflate(&strm, 1);				// Don't check the return value as, er, nothing can go wrong...
			int oldLen = newLen;
	...
	if (summaryNoteDir) {
		ssmap_Add(header, "X-MMTSLOGDIR", summaryNoteDir);
	}

	if (chunkSize) {
		ssmap_DeleteKey(header, "Content-Length");
		ssmap_Add(header, "Transfer-encoding","chunked");
	} else {
		if (len == -2) {
			ssmap_DeleteKey(header, "Content-Length");
		} else {
	...
	while (ssmap_GetNextEntry(header, &name, &value)) {
		headerBlock = hprintf(headerBlock, "%s: %s\r\n", name, value);
	}

	headerBlock = hprintf(headerBlock, "\r\n");

	MyBIO_write(io, headerBlock, strlen(headerBlock));	// Write the entire header
Log("Header:\n%s", headerBlock);
Log("Len = %d, content = %p, chunkSize=%d", len, content, chunkSize);
	if (len >= 0 && content) {
		if (chunkSize > 0) {							// Transfer-encoding: chunked
			const char *ptr = content;

			while (len > 0) {
				int size = chunkSize;
				if (size > len) size=len;
				SendHttpChunk(io, size, ptr);
Log("Chunk: %s", NiceBuf(size, ptr));

				ptr+=size;
				len-=size;
			}
			SendHttpChunk(io, 0, NULL);
		} else {										// Plain transfer-encoding
			MyBIO_write(io, content, len);
Log("Data: %s", NiceBuf(len, content));
		}
	}
...
void SendHttpBuffer(BIO *io, int code, SSMAP *header, HBUF *hbuf, int chunkSize = 0)
// Sends an HTTP message that is held in an HBUF*
// This function takes ownership of the buffer so don't go freeing it afterwards
{
	...
	SendHttp(io, code, header, len, content, chunkSize);
...
void TableNotes(BIO *io, note_t *note, const char *szDir)
{
	...
	const char *szDatetime = note_FindEntry(note, 'D', "unknown");
	...
	if (szInteraction) BIO_putf(io, "<tr><td bgcolor=#aa88ff>Interaction ID<td bgcolor=#ffffaa>%s\r\n", szInteraction);
	BIO_putf(io, "<tr><td bgcolor=#aa88ff>Message ID<td bgcolor=#ffffaa>%s\r\n", note_FindEntry(note, 'I', "none"));
	if (note_FindEntry(note, 'C')) 
...
void TableNoteMessages(BIO *io, note_t *note, const char *szDir)
{
	...
	for (i=1;i<=note_GetMessageCount(note);i++) {
		...
		BIO_putf(io, "<td bgcolor=#ffffaa><a href=\"/message/%s/%s.%s\">%s</a>",
...
// or that it is NULL or "default".
{
	...
	if (szEnv) {
		const char *szDir = hprintf(NULL, "env/%s", szEnv);
		bResult = isDir(szDir);
		szDelete(szDir);
	}
...
SSET *GetEnvironments()
// Returns a set of the available environments
// Always returns a valid SSET*, never NULL.
{
	static SSET *ss = NULL;
...
const char *GetEnvironment(const char *ip, int port)
...
{
	const char *szVar=NULL;										// The name of the entry in '.conf' used to find environment
	const char *szEnv=NULL;										// Our chosen environment

	SSET *envs = GetEnvironments();								// Get set of available environments
	if (!sset_Count(envs)) {
		Fatal("No environments found in %s/env", GetBaseDir());
	}

	if (ip && port) {
		szVar=hprintf(NULL, "environment-%s:%d", ip, port);
		szEnv = config_GetString(szVar);
		if (szEnv && !sset_Contains(envs, szEnv)) {
			Log("Environment for %s:%d configured as '%s', but it doesn't exist in %s/env - ignoring",
					ip, port, szEnv, GetBaseDir());
			szEnv = NULL;
		}
	}

	if (!szEnv && ip) {											// Port was 0 or no entry found for specific port
		szDelete(szVar);
		szVar=hprintf(NULL, "environment-%s", ip);
		szEnv = config_GetString(szVar);
		if (szEnv && !sset_Contains(envs, szEnv)) {
			Log("Environment for %s configured as '%s', but it doesn't exist in %s/env - ignoring",
					ip, szEnv, GetBaseDir());
			szEnv = NULL;
		}
	}
	...
	if (!szEnv) {												// Nothing specific found at all so take the default
		...
		szEnv = config_GetString(szVar);
		if (szEnv && !sset_Contains(envs, szEnv)) {
			Fatal("Default environment configured as '%s', but it doesn't exist in %s/env",
				szEnv, GetBaseDir());
		}
	}

	if (!szEnv) {												// Nothing for this address or port or even a default
		int nCount = sset_Count(envs);

		if (nCount > 1) {
			Log("Multiple environments available, but no default set");
			Log("Please set correctly in %s: environment=whatever", GetConfigFile());
			Fatal("Fix %s - add environment=whatever", GetConfigFile());
		}

		// If there is only one environment, silently choose it even if not configured
		szEnv = strdup(sset_GetStringAtIndex(envs, 1));
	}

	if (szVar) {
		Log("Environment set to '%s' for %s:%d (%s:%s)", szEnv, ip, port, GetConfigFile(), szVar);
		szDelete(szVar);
	}
...
{
Log("DeliverFile(%s, %s)", szFilename, szContentType);
	FILE *fp = fopen(szFilename, "r");
	...
	SSMAP *header = ssmap_New();
	ssmap_Add(header, "Content-Type", szContentType);
	SendHttp(io, 200, header, size, NULL, 0);
	ssmap_Delete(header);
...
//			2	Broken pipe (roughly equivalent to dropping the line)
{
	...
	const char *mesg = "Unknown error number";
	switch (nErr2) {
	case SSL_ERROR_NONE:				szErr="NONE";					mesg="protocol completed ok"; break;
	case SSL_ERROR_SSL:					szErr="SSL";					mesg="protocol failed - more info below"; break;
	case SSL_ERROR_WANT_READ:			szErr="WANT_READ";				mesg="Cannot complete before more data received"; break;
	case SSL_ERROR_WANT_WRITE:			szErr="WANT_WRITE";				mesg="Cannot complete before more data sent"; break;
	case SSL_ERROR_WANT_X509_LOOKUP:	szErr="WANT_X509_LOOKUP";		mesg="Further lookup required"; break;
	case SSL_ERROR_SYSCALL:				szErr="SYSCALL";				mesg="I/O error occured"; break;
	case SSL_ERROR_ZERO_RETURN:			szErr="ZERO_RETURN";			mesg="Connection closed"; break;
	case SSL_ERROR_WANT_CONNECT:		szErr="WANT_CONNECT";			mesg="Outgoing connection not yet made"; break;
	case SSL_ERROR_WANT_ACCEPT:			szErr="WANT_ACCEPT";			mesg="Incoming connection not yet made"; break;
	...
	Log("SSL Error ret=%d: err=%d (SSL_ERROR_%s = %s), errno = %d", ret, nErr2, szErr, mesg, errno);

	nErr3 = ERR_get_error();
	if (nErr3) {
		Log("Error stack as follows:");
		int level = 1;

		while (nErr3) {
			Log("SSL error level %d: %s", level++, ERR_error_string(nErr3, NULL));
			if (nErr3 == 0x14094416)
				Log("(The other end rejected my certificate)");
			nErr3 = ERR_get_error();
		}
	}
...
// If 0 then we will have called X509_STORE_CTX_set_error() with the error code.
{
	...
	szDelete(_tls_cname);
	_tls_cname = strdup(buf);

Log("VFY: Certificate check: depth=%d, err = %d, peer = '%s'", depth, err, _tls_cname);
	...
	// Ok, done that but it may be that it should be conditional on some configuration...
		struct in_addr cnAddress = GetHostAddress(_tls_cname);

		if (cnAddress.s_addr == INADDR_NONE) {
			Log("VFY: DNS lookup of connecting CN (%s) failed", _tls_cname, depth);
			err = X509_V_ERR_CERT_UNTRUSTED;
		} else if (strcmp(inet_ntoa(cnAddress), _szTlsPeerIp)) {
			Log("VFY: Connecting CN (%s) resolves to %s, while their IP address is %s", _tls_cname, inet_ntoa(cnAddress), _szSenderIp);
			err = X509_V_ERR_CERT_UNTRUSTED;
		}
	}
	...
	if (nSerial == 0x40a20fb6L)
		_tls_nhs = 1;
Log("Certificate %d serial number is %#x", depth, nSerial);

	if (depth >= SSL_MAXDEPTH) {
		Log("VFY: Peer certificate depth (%d) exceeds maximum (%d)", depth, SSL_MAXDEPTH);
	...
	if (tRevoked && tRevoked < time(NULL)) {
		Log("VFY: Peer certificate was revoked at %s", NiceTime(tRevoked));
...
//	const char *szValidTo = (const char *)peer->cert_info->validity->notAfter->data;
	const ASN1_TIME *validFrom = X509_get0_notBefore(peer);
	...
	if (tValidFrom > tNow) {
		Log("VFY: Peer certificate not valid until %s", NiceTime(tValidFrom));
		if (!err) err = X509_V_ERR_CERT_NOT_YET_VALID;
	}

	if (tValidTo < tNow) {
		Log("VFY: Peer certificate expired at %s", NiceTime(tValidTo));
		if (!err) err = X509_V_ERR_CERT_HAS_EXPIRED;
	}

	if (err || depth == 0)			// 0 is the base certificate, which is interesting and so is an error
		Log("VFY: SSL peer(%s) is valid %s - %s", _tls_cname, NiceTime(tValidFrom), NiceTime(tValidTo));

	X509_STORE_CTX_set_error(ctx, err);		// NB. Might be setting it to 0 when it wasn't 0 before

	if (err) {
		Log("VFY: verify error %d (%s) for %s", err, X509_verify_cert_error_string(err), _tls_cname);
		return 0;
	} else {
		return 1;
	}
...
BIO *BuildConnection(const char *szAddress, int nPort, int tls=-1, int wantBuffer=1)
// Builds a connection using SSL if the port number is 443
// tls = 0 (plain connection), 1 (TLS connection), -1 depends on port
// wantBuffer	1 to include a buffer, 0 not to (always get a buffer if we're using TLS)
{
...
//	Log("Making socket");
	...
	if (tls == -1)
		tls = (nPort == 443 || nPort == 444 || nPort == 1880); // 444 - QuickSilva's GP2DRS port usage, 1880 - NHS111
...
#if 1
	mydata_index = SSL_get_ex_new_index(0, (void *)"mydata index snd", NULL, NULL, NULL);
//Log("send mydata_index = %d", mydata_index);

//	SSL_CTX_set_verify(g_ctx,SSL_VERIFY_PEER,verify_callback_snd);
	_szTlsPeerIp = _szReceiverIp;
...
int RawSendMessage(BIO *io, MSG *msg, int bAck, int nCode)
...
//		1		Didn't send due to an option in the option file
{
	...
	szMessage = MakeHttpMessage(msg, bAck, nCode);
...
int SendAcknowledgementMessage(BIO *io, MSG *msg)
{
	...
	return RawSendMessage(io, msg, 1, nCode);
}
...
// In here, we decide what sort of message it is and call a corresponding script/binary in the handlers directory
{
	...
	if (rxData1) {
		...
		char *colon = (char *)strrchr(handler, ':');	// NB. Effective quite <const_cast> going on here

		while (access(handler, 1) && colon && colon > slash) {				// While the handler isn't an executable
			*colon='\0';
			colon = (char *)strrchr(handler, ':');
		}
...
// Returns 1 if we've fallen back to the old '.waiting' method
{
	...
	const char *interactionId = msg_GetInteractionId(msg);

	if (interactionId) {
		// We don't return application level acknowledgements to the application (1.79)
		if (!strcasecmp(interactionId, "Acknowledgment") || !strcasecmp(interactionId, "Acknowledgement"))
			return 0;
	}
...
//Log("IsAck = %d, HasPayload = %d", msg_IsAck(msg), msg_HasPayload(msg));
	...
	if (msg_HasPayload(msg)) {							// Has a payload that we'd better deliver
		...
		if (szId && HaveReceived(szDuplicateCheckId)) {						// Duplicate
			...
			if (!szId) {
				Log("Received message with no ID so ID is made up - no point in duplicate checking");
			}
	...
	const char *interactionId = msg_GetInteractionId(msg);
	if (interactionId && !strcmp(interactionId, "Ping" )) {
...
// docs/abc/def.sh, docs/abc, docs/abc.pl, docs/abc.php, docs/abc.sh
{
	...
	if (szExecutable) {
		...
		if (!count) {
			...
			szTempDir = hprintf(NULL, "/usr/mt/tmp/mmts_dir_%d_%u", getpid(), time(NULL));
			MkDir(szTempDir);
			int part;
		...
		szStdin = hprintf(NULL, "/usr/mt/tmp/mmts_stdin_%d_%u", getpid(), time(NULL));
		fp=fopen(szStdin, "w");
		if (fp) {
...
// A duff string will set us up with '0, NULL'.
{
	...
	if (chp) {								// Otherwise NULL, which works out fine
		...
		++chp;
		chp2=(char *)strchr(chp, '\n'); if (chp2) *(char*)chp2='\0';
		chp2=(char *)strchr(chp, '\r'); if (chp2) *(char*)chp2='\0';
	}
...
MIME *HttpRequest(const char *szAddr, int nPort, const char *requestLine, MIME *request, const char **pStatusLine)
// Send a request to a server and return a result.
// The status line returned by the remote server will be placed in *pStatusLine (on the heap)
// Does the 100-continue protocol bit if there is an 'expect' in the header
{
	BIO *io = BuildConnection(szAddr, nPort);
	if (!io) {
		if (pStatusLine) *pStatusLine = strdup(http_StatusLine(503));
	...
	mime_SetHeader(request, "Host", szAddr);			// Ensure the host header is set correctly

	int contentLength = 0;
	const char *body = mime_RenderContentBinary(request, &contentLength);
	char contentLengthStr[20];
	snprintf(contentLengthStr, sizeof(contentLengthStr), "%d", contentLength);
	mime_SetHeader(request, "Content-Length", contentLengthStr);
	const char *headers = mime_RenderHeaderHeap(request);

	const char *expect = mime_GetHeader(request, "Expect");
	bool needContinue = expect && !!stristr(expect, "100-Continue");

Log("Sending to %s:%d", szAddr, nPort);
Log("Sending as: %s\n", requestLine);
Log("Sending headers (%d):\n'%s'", strlen(headers), headers);
Log("Sending Body (%d): %.50s%s", contentLength, body, strlen(body)>50?"...":"");

	char buf[200];										// For the initial response line
	MIME *m = NULL;
	*buf='\0';
	bool continueFailed = false;

	MyBIO_puts(io, requestLine);							// E.g. GET / HTTP/1.1
	MyBIO_puts(io, "\r\n");
	MyBIO_puts(io, headers);							// All the headers
	MyBIO_puts(io, "\r\n");
	MyBIO_flush(io);

	if (needContinue) {									// We need to wait for a HTTP/1.1 100 Continue or an error
		char buf[200];
		static const int timeout = 1;
		bool timedout = false;							// true if we've timed out

		// We'll only wait a second for the continue response before sending the body anyway
		_nAlarmNumber=6;
		if (setjmp(jmpbuf_alarm))
			timedout = true;

		if (timedout) {
Log("Timed out waiting for 'continue' from peer");
		} else {
			alarm(timeout);								// Give them 1 second to respond with a '100 Continue'

Log("Waiting for a 'Continue' from the peer");
			MyBIO_flush(io);
			MyBIO_gets(io, buf, sizeof(buf));

			http_ParseReceivedHeader(buf);
			int status = http_GetLastStatusCode();

			if (status == 100) {
				char blankline[256];
				*blankline = '!';										// Anything to stop it being blank

				while (*blankline != '\r' && *blankline != '\0') {		// Step over any header until we hit the blank line
					MyBIO_gets(io, blankline, sizeof(blankline));
				}
			} else {
				continueFailed = true;
				Log("Expected 100 Continue, got %d", status);
			}
		}
		alarm(0);
	}
...
Log("Sending body now");
	...
	if (pStatusLine) *pStatusLine = strdup(buf);
...
//Log("BEFORE: %s", szBody);
				...
				char *szNewBody = (char *)malloc(start+strlen(szInsert)+rest);	// A whole new body
...
int PlainProxy(BIO *io, const char *verb, const char *url)
// Set up a plain proxying to the address given.
// We simply connect to the destination address, receive a request from 'io' and send it on to the
// destination.  We then send the result back to io and we're done.
// We now have more work to do in the case of an 'expect: 100-Continue' header
{
	const char *protocol;
	const char *address;
	int port;
	...
	if (!SplitEndpoint(url, &protocol, &address, &port, &uri, 1)) {
		Fatal("Malformed proxy request");
	}

	MIME *mimeOut;
	mime_ParseHeaderValues(0);
	mimeOut=GetIOMIMEHeader(io);
	if (!mimeOut)
	...
	if (!ioOut) {
		Log("Bad proxy request - cannot open connection to %d:%d", address, port);
		SendHttp(io, 400);
		return 0;
	}

	const char *requestLine = hprintf(NULL, "%s %s HTTP/1.1", verb, uri);

	mime_SetHeader(mimeOut, "Host", address);

	if (strcmp(verb, "GET") && strcmp(verb, "HEAD") && strcmp(verb, "DELETE") && strcmp(verb, "OPTIONS")) {
		const char *expect = mime_GetHeader(mimeOut, "Expect");

		if (stristr(expect, "100-Continue")) {
//			SendHttp(io, 100);							// This method also sends some header lines (Content-Length, Date etc)
			MyBIO_puts(io, HttpHeader(100));
			MyBIO_puts(io, "\r\n");
		}

		mimeOut = mime_ReadBody(mimeOut);

//Log("Body received = '%s'", mime_RenderHeap(mimeOut));
	}

	const char *statusLine;
	Log("Forwarding request to %s:%d", address, port);
	Log("Sending: %s", requestLine);
	MIME *mimeIn = HttpRequest(address, port, requestLine, mimeOut, &statusLine);
	...
	http_ParseReceivedHeader(statusLine);
	int redirects = 0;
	int status = http_GetLastStatusCode();
	while (status >= 300 && status <= 308) {	// Too general, but we're being redirected
		redirects++;
		if (redirects > 10)
			Fatal("Too many redirects");

		const char *szLocation = mime_GetHeader(mimeIn, "Location");

		if (szLocation) {
			const char *szPrevLocation = hprintf(NULL, "%s%s", address, uri);
			const char *szRedirProtocol;
			const char *szRedirAddr;
			int nRedirPort = 80;
			const char *szRedirUri;

			Log("Redirect (%d) to %s", status, szLocation);
			...
			const char *szRedirHeader = hprintf(NULL, "%s %s HTTP/1.1", verb, szRedirUri);
			mime_SetHeader(mimeOut, "Host",szRedirAddr);
			mime_Delete(mimeIn);
			Log("Redirected - sending to %s:%d instead", szRedirAddr, nRedirPort);
			mimeIn = HttpRequest(szRedirAddr, nRedirPort, szRedirHeader, mimeOut, &statusLine);

			szDelete(szPrevLocation);
			status = http_GetLastStatusCode();
		} else {
			status = 400;
		}
	}
	...
	if (mime_GetLastError()) {
		Log("Malformed MIME received from proxy destination (error %d - %s)", mime_GetLastError(), mime_GetLastErrorStr());
	}
//	shutdown(fd, SHUT_WR);
//	close(fd);
//	BIO_close(ioOut);
	...
	if (summaryNoteDir) {
		mime_SetHeader(mimeIn, "X-MMTSPROXYLOGDIR", summaryNoteDir);
	}

	int len;
	const char *body = mime_RenderContentBinary(mimeIn, &len);
	const char *header = mime_RenderHeaderHeap(mimeIn);

	Log("Sending response back to the client (%s)", statusLine);

	MyBIO_puts(io, statusLine);
	MyBIO_puts(io, "\r\n");
	if (header) MyBIO_puts(io, header);
	MyBIO_puts(io, "\r\n");
	MyBIO_write(io, body, len);
...
int WampConnected(WAMP *wamp, const char *message)
{
	Log("Connected %p - '%s'", message);
...
int DealWithGET(BIO *io, const char *szURI)
{
	...
	if (!mime) Log("Bad MIME on HTTP GET (%s)", mime_GetLastErrorStr());

	SSMAP *headerMap = SetIncomingHeaderFromMime(mime);
...
//Log("Upgrade = '%s'", szUpgrade);

		if (szUpgrade && !stricmp(szUpgrade, "websocket")) {
			wamp_AcceptBio(daemon_channel_pool, io, WampConnected, headerMap);
			return 0;
		}
	}
...
//		bprintf(h, "<tr><td>Connections on TLS/SSL port<td bgcolor=#eeeeff>%d", _nExternalCount);
		...
		szDelete(szIconFile);
	} else if (!strncasecmp(szAddr, "/logdir/", 8)) {
		const char *name = szAddr+8;						// 17/06/11/abcde.zip
		regex_t preg;
		const char *match = "^[0-2][0-9]/[0-1][0-9]/[0-3][0-9]/[a-z][a-z][a-z][a-z][a-z]\\.zip$";

Log("Checking '%s' against '%s'", name, match);
		int err = regcomp(&preg, match, REG_NOSUB);
		if (err) {
			char buf[50];

			regerror(err, &preg, buf, sizeof(buf));
			Log("Regex error in '%s' (%s) - skipping", match, buf);
		} else {
			if (!regexec(&preg, name, 0, NULL, 0)) {
				const char *tmp = tempnam(NULL, "mmtsl");
				const char *tmpfile = hprintf(NULL, "%s.zip", tmp);
				const char *cmd = hprintf(NULL, "zip -jqr %s %s/%.14s", tmpfile, szMsgLogDir, name);
Log("Cmd: %s\n", cmd);
				system(cmd);
				DeliverFile(io, tmpfile, NULL);

				unlink(tmpfile);
				szDelete(tmpfile);
				szDelete(tmp);
			} else {
				Log("Doesn't look like a proper request for a log file (%s)", name);
			}
			regfree(&preg);
		}

	} else if (!strncasecmp(szAddr, "/message/", 9)) {
		const char *filename = hprintf(NULL, "%s/%s", szMsgLogDir, szAddr+8);
		DeliverFile(io, filename, NULL);
		szDelete(filename);
...
//			szDelete(szCommandLine);
		...
		if (fp) {
			MyBIO_puts(io, http_StatusLine(200));
			MyBIO_puts(io, "\r\n");
...
//BIO_putf(io, "Content-type: text/html\r\n\r\nCommand line = '%s'<hr>", szCommandLine);
			...
			SendHttp(io, 404, NULL, -1, "Unrecognised URI");
		}
...
// Mantis 15333 - I'm unsure whether this approach is correct on incoming NASP, but I can't really see why not
		...
		if (!RawSendMessage(io, msgReply, 0, 200)) {
...
void CloseProxyChannel(CHAN *chan)
// If one proxy channel goes, the other must go too...
{
	if (chan) {
		CHAN *other = (CHAN*)chan_Info(chan);
		Log("Channel %s is being deleted, %s must go too", chan_Name(chan), chan_Name(other));

		chan_Delete(other);
	}
}

int DealWithCONNECT(BIO *io, const char *szURI)
// We have a CONNECT request so we will connect the caller with the given URI
// https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods/CONNECT
// We connect to the URI, then forward all traffic in both directions until one side drops.
{
	CHAN *chanIn;
...
Log("CONNECT instruction to %s", szURI);

	mime_ParseHeaderValues(0);
	MIME *mime=GetIOMIMEHeader(io);
	if (!mime) {
		Log("Invalid header with CONNECT request");
		SendHttp(io, 400);
	}

	SplitEndpoint(szURI, &protocol, &address, &port, &uri, true);
	BIO *ioOut=BuildConnection(address, port, 0, 0);	// Give us a connection
//case 407: s="Proxy Authentication Required"; break;
	...
	CHANPOOL *pool;
	if (0) {									// Use existing pool and channel
		...
		chanIn = chan_FindByFd(pool, fd);
		Log("Channel(%d) = %s", fd, chan_Name(chanIn));
	} else {
		pool = chan_PoolNew();
		chanIn = chan_NewBio(pool, io, CHAN_IN | CHAN_OUT);
Log("Incoming channel %s, BIO = %p", chan_Name(chanIn), io);
	}
	CHAN *chanOut = chan_NewBio(pool, ioOut, CHAN_IN | CHAN_OUT);
Log("Outgoing channel %s, BIO = %p", chan_Name(chanOut), ioOut);

	chan_Connect(chanIn, chanOut);
	chan_Connect(chanOut, chanIn);
	chan_SetInfo(chanIn, chanOut);
	chan_SetInfo(chanOut, chanIn);
	chan_OnDelete(chanIn, CloseProxyChannel);
	chan_OnDelete(chanOut, CloseProxyChannel);

	chan_Logger(vLog);
	SendHttp(io, 200, NULL, -2);							// Tell our connecting channel that everything's good

	Log("Connected incoming channel with %s:%d", address, port);
	chan_EventLoop(pool);
	Log("Connections broken");
...
Log("resultList = %s", json_Render(resultList));
Log("resultDict = %s", json_Render(resultDict));

		if (json_IsObject(resultDict)) {
			httpCode = json_ObjectIntegerCalled(resultDict, "http_code", 0);
...
Log("Got content %d", contentLength);
	if (contentLength) {
		SendHttp(fhir_io, httpCode, header, contentLength, content, contentLength > 10240 ? 10240 : 0);
		NoteMessage(content, contentLength, "Body-out", "*", "Body Sent");
	} else {
...
int OnWampConnection(WAMP *wamp, const char *mesg, void *data)
{
	if (wamp) {
	...
	JSON *headerObject = json_NewObject();
	const char *headerName;
	for (headerName = szzHeaders; *headerName; headerName = szz_Next(headerName)) {
		char *name = strdup(headerName);
		strlwr(name);

		const char *value = mime_GetFullHeaderValue(mime, headerName);

		json_ObjectAddStringz(headerObject, name, value);

		szDelete(name);
		szDelete(value);
	}

	if (_incoming_networkport) {
		char buf[6];
		snprintf(buf, sizeof(buf), "%d", _incoming_networkport->nPort);

		json_ObjectAddStringz(headerObject, "x-mt-transport", ProtocolName(_incoming_networkport->nProtocol));
		json_ObjectAddStringz(headerObject, "x-mt-port", buf);
	} else {
		json_ObjectAddStringz(headerObject, "x-mt-transport", "unknown");
		json_ObjectAddStringz(headerObject, "x-mt-port", "0");
	}

	json_ObjectAddStringz(headerObject, "x-mt-tls", _tls_version);
	json_ObjectAddStringz(headerObject, "x-mt-cipher", _tls_cipher);
	json_ObjectAddStringz(headerObject, "x-mt-cname", _tls_cname);
	json_ObjectAddStringz(headerObject, "x-mt-nhs", _tls_nhs ? "1" : "0");

	json_ObjectAddStringz(argDict, "headers", json_Render(headerObject));
	json_Delete(headerObject);

//Log("MIME: '%s'", mime_RenderHeaderHeap(mime));
	int len;
	const char *body = mime_RenderContentBinary(mime, &len);
	NoteMessage(body, -1, "Body-in", "*", "Body Received");
...
//Log("HTTP Request: '%s' : '%s' : '%s'", szMethod, szURI, szVersion);
	...
	bool done = false;				// Set if we handle this with the api_redirect file

	if (!strncasecmp(szURI, "/https://", 9) || !strncasecmp(szURI, "/http://", 8)) {
		if (strcmp(_szIncomingIp, "127.0.0.1")) {
			Log("Proxy requests should only be accepted from localhost (not %s)", _szIncomingIp);
//			Fatal("Proxy requests can only be accepted from localhost (not %s)", _szIncomingIp);
		}

		return PlainProxy(io, szMethod, szURI+1);
	}

	const char *wampRedirect = hprintf(NULL, "%s/api_redirect.map", GetEtcDir());
	FILE *fp = fopen(wampRedirect, "r");
	if (fp) {
		regex_t preg;
		const char *line;

		while ((line = ReadLine(fp))) {
			char *name = SkipSpaces(line);
			if (!*name || *name == '#') continue;			// Blank line or comment

			char *value = strchr(name, '=');
			if (!value) continue;							// No '=' on the line, skip it quietly
			*value++='\0';

//Log("Checking '%s' '%s' against '%s'", name, value, szURI);
			int err = regcomp(&preg, name, REG_NOSUB);
			if (err) {
				char buf[50];

				regerror(err, &preg, buf, sizeof(buf));
				Log("Regex error in '%s' (%s) - skipping", name, buf);
				continue;
			}

			if (!regexec(&preg, szURI, 0, NULL, 0)) {
//Log("ApiRedirect(%s): %s", name, value);
				n = DealWithApiRedirect(io, szMethod, szURI, value);
				done = true;
				break;
			}
			regfree(&preg);
	...
	if (!done) {
		...
		} else if (!strcasecmp(szMethod, "CONNECT")) {
			n=DealWithCONNECT(io, szURI);
		} else {
...
static void ServeRequest(BIO *io, int fd, bool https)
// Talk to the Client that's calling us
{
	...
	if (nGot) {
		...
		if (szMethod && szURI && szVersion) {
			...
			int reject = firewall_ConnectionDenied(fd, &rejection, https, szMethod, szURI);
			if (reject) {
				Log("Connection rejected (%d): %s", reject, rejection);
				if (reject > 0) {
					SendHttp(io, reject);
				}
			} else {
				Log("Firewall: %s", rejection);
...
szCheck="A";
	...
	case 'V': case 'A':							// Verify or Ask
		Log("Asking for certificate");
		nMode = SSL_VERIFY_PEER;
		break;
	case 'M':									// Must	- Seems to block browsers...
		Log("Requiring certificate");
		nMode = SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT;
		break;
	case 'O':									// Once
		Log("Requiring certificate once");
		nMode = SSL_VERIFY_PEER | SSL_VERIFY_CLIENT_ONCE;
		break;
	case 'N':									// None or No
		Log("Not asking for certificate");
	...
	_szTlsPeerIp = _szSenderIp;
	SSL_set_verify(ssl,nMode,verify_callback_rcv);

	SSL_set_min_proto_version(ssl, TLS1_VERSION);		// Accept nothing less than TLS 1.0
	SSL_set_cipher_list(ssl, "AESGCM+EECDH:AESGCM+EDH:AES256+EECDH:AES256+EDH:ALL");

//Log("---------- About to attempt to accept the connection (SSL=%p, ctx=%p)", ssl, ctx);
// All decisions on making / rejecting connection should come here
	if (((r=SSL_accept(ssl)) <= 0)) {		// 1 is success, anything below is a problem
		if (r == 0) {
			Log("TLS connection terminated by peer");
		} else {
			Log("TLS dirty shutdown (%d)", r);
		}

		int nOk = LogSslError(ssl,r);
		NoteInhibit(1);						// Never record this in individual logs
	...
	for (int i=0;i<nCiphers;i++) {
		...
		const char *name = SSL_CIPHER_get_name(fred);
		Log("Offered cipher '%s'", name);
	}
#endif

	szDelete(_tls_version);
	_tls_version = strdup(SSL_get_version(ssl));

	const SSL_CIPHER *cipher = SSL_get_current_cipher(ssl);
	szDelete(_tls_cipher);
	_tls_cipher = strdup(SSL_CIPHER_get_name(cipher));

	Log("Accepted %s connection using %s", _tls_version, _tls_cipher);

	RememberEnvironment(_szSenderIp, _nIncomingPort, szEnvironment, 1);	// Assert that this barbeque tastes good...
// This is a good resource: https://wiki.openssl.org/index.php/SSL/TLS_Client
	...
	if (peer) {
		...
		if (verifyError != X509_V_OK) {
			Log("*** Error %lx verifying peer", verifyError);
		}

		X509_NAME_get_text_by_NID(X509_get_subject_name(peer), NID_commonName, cn, sizeof(cn));
		g_ssl_cn = strdup(cn);

		X509_NAME_oneline(X509_get_subject_name(peer), parent, sizeof(parent));
		Log("Peer '%s' issued by '%s'", cn, parent);
...
// the log AFTER the result is returned.  This should be the stderr of the executed binary.
{
	...
	if (stdin_buf) {
		...
		do {
			...
			w += written;
		} while( written > 0 && w != (int)strlen(stdin_buf) );
	}
	close( fdin[1] );
	...
	if( (pid = spiderFork("NAME-LEG")) == 0 ) {
		...
		if( stdin_buf != NULL ) {
			do {
				...
				w += retval;
			} while( retval > 0 && w != (int)strlen(stdin_buf) );
		}
		close( fd1[1] );
...
//			NULL			No file found
{
	...
	szDisabled = (char *)malloc(nMaxNameLen);	// Filename of disablement file

	snprintf(szDisabled, nMaxNameLen, "%s/DISABLED", szOutDir);

	if (access(szDisabled, 0)) {					// Not been disabled
		char *szSrcName = (char *)malloc(nMaxNameLen);
		char *szDestName = (char *)malloc(nMaxNameLen);
		struct dirent *d;
		DIR *dir=opendir(szOutDir);
		time_t now = time(NULL);
...
//		if (!szNewTimeServers) szNewTimeServers=strdup("cns0.nhs.uk cns1.nhs.uk");
		if (!szNewTimeServers) szNewTimeServers=strdup("pool.ntp.org");
	...
	if (bIsDaemon) {
			...
			if (!nTimerPid) {								// Not currently running the timer
				if (nTimeTrack > nTimeCheck) {				// The time to do a time check is nigh
					...
					if (!nTimerPid) {						// We're the child process
						...
						const char *param[20];				// NB. Value in loop below must be one less than this
						int i=0;
						char *arg;
						...
						int err=execv(param[0], (char * const *)param);
		...
		while ((childpid = waitpid(-1, &status, WNOHANG)) > 0) {
			...
			if (childpid == nTimerPid) {					// Timer process terminating so deal with it
				...
				nTimeCheck = nTimeTrack+7200;			// Check again in a couple hours
	...
	if (bIsDaemon) {
		...
		if (nPids) {
			...
			for (i=0;i<nPids;i++) {					// Ask all the children to commit suicide
				if (aPid[i].pid > 0) {		// Something is allowing this to be 0 not sure how (RJ 18-04-2017)
											// And we don't want to kill ourselves as we'll loop
					Log("Sending kill %s to %d", SignalName(3), aPid[i].pid);
			...
			for (i=0;i<nPids;i++) {					// Slaughter any that are still alive
				if (aPid[i].pid > 0) {
					Log("Sending kill %s to %d", SignalName(9), aPid[i].pid);
			...
			Idle();									// Note their deaths...
			if (nPids) {
				Log("I think we still have offspring: %d", nPids);
				for (i=0;i<nPids;i++) {
					const char *szIp = aPid[i].szIp;
					const char *szId = aPid[i].szId;

					Log("Child %d: pid=%d, started %s, ip=%s, id=%s",
							i,
							aPid[i].pid,
							NiceTime(aPid[i].tStarted),
							szIp ? szIp : "None",
							szId ? szId : "None");
				}
			}
...
{
	_incoming_networkport = (NetworkPort_t*)chan_Info(chan);
	int fd;
	int childPid;
...
#endif
	_incoming_networkport->nCount++;
	int nSrcProtocol=_incoming_networkport->nProtocol;
	int nSock=_incoming_networkport->nSock;

	if ((fd=accept(nSock,0,0)) < 0) {
		Log("Problem accepting (errno=%d) on port %d", errno, _incoming_networkport->nPort);
		return 1;
	}
	...
	// Specific code to drop connections from 10.198.166.134 as it pokes rfa every 30 seconds and makes log examination problematic
//	if (!strcmp(_szSenderIp, "10.198.166.134")) {
//		Log("Just say no to 10.198.166.134 as it pokes us every 30 seconds");
//		return 1;
//	}
	...
	Log("Connection %d on channel %s - %s from %s on port %d", _incoming_networkport->nCount, chan_Name(chan), ProtocolName(nSrcProtocol), _szSenderIp, _incoming_networkport->nPort);

	const char *firewallFail;
	if (firewall_ConnectionDenied(fd, &firewallFail)) {
		Log("Connection dropped: %s", firewallFail);
		shutdown(fd, SHUT_RDWR);
		close(fd);
		return 0;
	} else {
		Log("Firewall: %s", firewallFail);
	}


#if 0
	Log("sock = %s", Printable(sizeof(struct sockaddr), (const char *)&sin));

	struct sockaddr_storage faddr;
	socklen_t               faddrlen = sizeof(sin);
	char                    hname[NI_MAXHOST];
	    char                    sname[NI_MAXSERV];
	 int error = getnameinfo((struct sockaddr *)&sin, faddrlen, hname, sizeof (hname), sname, sizeof (sname), 0);
	 if (error) {
		Log("getnameinfo: %s", gai_strerror(error));
	 } else {
		 Log("Connection from %s/%s", hname, sname);
	 }
	...
	int err2 = ioctl(nSock, SIOCGIFCONF, &ifc);
	Log("A %d - %d %s (errno=%d)", err2, ifc.ifc_len, Printable(sizeof(ifc), (const char *)&ifc), errno);
	if (!err2) {
		ifc.ifc_buf = (char*)malloc(ifc.ifc_len);
		int err3 = ioctl(nSock, SIOCGIFCONF, &ifc);

		Log("B %d - %d %s", err3, ifc.ifc_len, Printable(ifc.ifc_len, ifc.ifc_buf));
		...
		for (int i=0; i<ifcount; i++) {
			...
			if (r->ifr_addr.sa_family == AF_INET) {
				...
				int ierr = ioctl(fd, SIOCGIFNETMASK, &conf);
				Log("I ioctl(%d, %x, %p) = %d - errno = %d", fd, SIOCGIFNETMASK, &conf, ierr, errno);
    ...
    strncpy(conf.ifr_name, "*", IFNAMSIZ - 1); // eth0 as an interface
    conf.ifr_addr.sa_family = AF_INET; //ipv4

	int ierr = ioctl(fd, SIOCGIFNETMASK, &conf);
	Log("I ioctl(%d, %x, %p) = %d - errno = %d", fd, SIOCGIFNETMASK, &conf, ierr, errno);
	unsigned char* netmask = (unsigned char *)conf.ifr_ifru.ifru_broadaddr.sa_un.sa_s.sa_saus_data+2; // pointer zur subnetzmaske
	    Log("Subnet mask : %u.%u.%u.%u", netmask[0], netmask[1], netmask[2], netmask[3]);

	ierr = ioctl(nSock, SIOCGIFNETMASK, &conf);
	Log("J ioctl(%d, %x, %p) = %d - errno = %d", nSock, SIOCGIFNETMASK, &conf, ierr, errno);
...
void BeDaemon()
{
	...
	chan_PoolCallEvery(daemon_channel_pool, 500, Idler, NULL);
...
// bRestart indicates that we expect a server to be already running
{
	...
	const char *firewallFail = firewall_Load(0);
