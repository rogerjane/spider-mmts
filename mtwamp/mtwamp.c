...
typedef struct client_subscription_t {						// For recording subscriptions for the client
	...
	int type;												// WAMP_EXACT, WAMP_PREFIX or WAMP_WILDCARD
...
API void (*wamp_GetLogger())(const char *, va_list)
{
	return _Logger;
}
...
{
	if (type == WAMP_EXACT)		return "exact";
	if (type == WAMP_PREFIX)	return "prefix";
	if (type == WAMP_WILDCARD)	return "wildcard";
...
STATIC int wamp_Send(WAMP *wamp, const char *message)
...
{
	const char *message = json_RenderHeap(json, 0);
	if (message) {
//Log("WA: wamp_SendJson(%p, %s)", wamp, message);
		if (wamp->connectionType == WAMP_CONN_CODE) {
			(wamp->connection.fn)(wamp, json);
			json_Delete(json);
		} else {
			wamp_SendHeap(wamp, message);
		}
	} else {
...
// mesg MUST be on the heap and becomes owned by this function
{
	...
	wamp_SendError(wamp, type, id, NULL, uri, json_NewStringzHeap(mesg), NULL);
}
...
// NB. The callee may be deleted by this call!
{
	...
	wamp_PublishIn(wamp, "wamp.registration.on_unregister", NULL, NULL, publishArgs);
...
//Log("Removing invokee %d for %s", index, callee->procedure);
	...
	if (callee->nInvokees) {
		...
		json_ObjectAddInteger(publishArgs, "session", wamp->session->id);
		json_ObjectAddInteger(publishArgs, "registration", registrationId);
		wamp_PublishIn(wamp, "wamp.registration.on_delete", NULL, NULL, publishArgs);
...
Log("WA: Deleting %s (%d)", w->name, w->deleting);
		...
		client_subscription_t *sub = idmap_GetValueAtIndex(w->clientSubscriptionsReq, 0);
		while (sub) {
		...
		client_publish_t *pub = idmap_GetValueAtIndex(w->clientPublishes, 0);
		while (pub) {
...
//Log("WA: wamp_RegisterAnyCallee(%p, %p, %s, %s, %Ld, %s, %p, %p)", realm, wamp, match, invoke, priority, procedure, invokee,  registrationIdp);
	...
	if (!callee) {													// Not had registration for this before
		...
		json_ObjectAddInteger(publishArgs, "session", wamp ? wamp->session->id : 0);
		json_ObjectAdd(publishArgs, "registrationDetails", registrationDetails);
		wamp_PublishIn(wamp, "wamp.registration.on_create", NULL, NULL, publishArgs);
	} else {
...
//		json_ObjectAdd(jFeatures, "call_canceling", json_NewBool(1));
//		json_ObjectAdd(jFeatures, "call_trustlevels", json_NewBool(1));
//		json_ObjectAdd(jFeatures, "session_meta_api", json_NewBool(1));					// Part done
...
STATIC int wamp_HandleWelcome(WAMP *wamp, JSON *json)
{
	...
	wamp->session = wamp_NewSession(wamp->realm, sessionId);
...
STATIC int wamp_HandleRegister(WAMP *wamp, JSON *json)
{
	...
	json_ObjectAddInteger(publishArgs, "session", wamp->session->id);
	json_ObjectAddInteger(publishArgs, "registration", registrationId);
	wamp_PublishIn(wamp, "wamp.registration.on_register", NULL, NULL, publishArgs);
...
STATIC int wamp_HandleRegistered(WAMP *wamp, JSON *json)
{
	if (wamp && json) {
		...
		client_registration_t *reg = idmap_GetValue(wamp->clientRegistrationsReq, requestId);
		if (reg) {
...
{
	wamp_SendInternalError(wamp, WAMP_CALL, callRequestId, errNoSuchProcedure, "Cannot do wamp.registration.lookup yet");
	return 1;
}

STATIC int wamp_HandleWampCall(WAMP *wamp, long long callRequestId, SPMAP *options, const char *procedure, JSON *argList, JSON *argDict)
{
	if (!strcmp(procedure, "wamp.registration.list")) {
		...
		wamp_SendInternalError(wamp, WAMP_CALL, callRequestId, errNoSuchProcedure, "Cannot do wamp.registration.match yet");
	} else if (!strcmp(procedure, "wamp.registration.get")) {
		wamp_SendInternalError(wamp, WAMP_CALL, callRequestId, errNoSuchProcedure, "Cannot do wamp.registration.get yet");
	} else if (!strcmp(procedure, "wamp.registration.list_callees")) {
		wamp_SendInternalError(wamp, WAMP_CALL, callRequestId, errNoSuchProcedure, "Cannot do wamp.registration.list_callees yet");
	} else if (!strcmp(procedure, "wamp.registration.count_callees")) {
		wamp_SendInternalError(wamp, WAMP_CALL, callRequestId, errNoSuchProcedure, "Cannot do wamp.registration.count_callees yet");
	} else if (!strcmp(procedure, "wamp.subscription.list")) {
		wamp_SendInternalError(wamp, WAMP_CALL, callRequestId, errNoSuchProcedure, "Cannot do wamp.subscription.list yet");
	} else if (!strcmp(procedure, "wamp.subscription.lookup")) {
		wamp_SendInternalError(wamp, WAMP_CALL, callRequestId, errNoSuchProcedure, "Cannot do wamp.subscription.lookup yet");
	} else if (!strcmp(procedure, "wamp.subscription.match")) {
		wamp_SendInternalError(wamp, WAMP_CALL, callRequestId, errNoSuchProcedure, "Cannot do wamp.subscription.match yet");
	} else if (!strcmp(procedure, "wamp.subscription.get")) {
		wamp_SendInternalError(wamp, WAMP_CALL, callRequestId, errNoSuchProcedure, "Cannot do wamp.subscription.get yet");
	} else if (!strcmp(procedure, "wamp.subscription.list_subscribers")) {
		wamp_SendInternalError(wamp, WAMP_CALL, callRequestId, errNoSuchProcedure, "Cannot do wamp.subscription.list_subscribers yet");
	} else if (!strcmp(procedure, "wamp.subscription.count_subscribers")) {
		wamp_SendInternalError(wamp, WAMP_CALL, callRequestId, errNoSuchProcedure, "Cannot do wamp.subscription.count_subscribers yet");
	} else {
...
STATIC int wamp_HandleInvocation(WAMP *wamp, JSON *json)
{
	...
	client_registration_t *reg = idmap_GetValue(wamp->clientRegistrationsReg, registrationId);
...
//Log("WA: INVOCATION: reg = (%p)", reg);
	...
	if (reg) {
		if (reg->cbInvoke) {
			...
			JSON *yield = (*reg->cbInvoke)(wamp, requestId, registrationId, procedure, details, list, dict);
...
STATIC int wamp_HandleResult(WAMP *wamp, JSON *json)
{
	...
	client_call_t *call = idmap_GetValue(wamp->clientCalls, requestId);
	if (call) {
		if (call->cbResult) {
			const char *procedure = json_ObjectStringzCalled(details, "procedure");
			if (!procedure) procedure = call->procedure;

			(*call->cbResult)(wamp, requestId, call->procedure, NULL, details, list, dict);
		}
...
STATIC int wamp_HandleSubscribed(WAMP *wamp, JSON *json)
{
	...
	client_subscription_t *sub = idmap_GetValue(wamp->clientSubscriptionsReq, requestId);
	if (sub) {
		...
		if (sub->cbSubscribed)
			(*sub->cbSubscribed)(wamp, requestId, subscriptionId, NULL, NULL);
	} else {
...
STATIC int wamp_HandleUnsubscribed(WAMP *wamp, JSON *json)
{
	...
	client_subscription_t *sub = idmap_GetValue(wamp->clientSubscriptionsUnreq, requestId);
	if (sub) {
...
// When we've done that, run through the list, sending a message to each WAMP.
{
	...
	int bExcludeMe = json_ObjectBoolCalled(options, "exclude_me");	// Defaults to -1, which conveniently equates to true
	...
	if (subs) {
		...
		while (subs) {
			...
			while (spmap_GetNextEntry(sub->sub->wamps, &wampName, NULL)) {
				...
				if (wRecip && (!bExcludeMe || wRecip != wamp)) {	// Send it the wamp still exists and it's not us
...
// Returns 1 if we called the callback
{
	...
	client_publish_t *pub = idmap_GetValue(wamp->clientPublishes, requestId);
	if (pub) {
		if (pub->cbPublished)
...
STATIC int wamp_HandleEvent(WAMP *wamp, JSON *json)
{
	...
	if (wamp) {
		...
		client_subscription_t *sub = idmap_GetValue(wamp->clientSubscriptionsSub, subscriptionId);
		if (sub) {
			if (sub->cbEvent) {
				const char *topic = json_ObjectStringzCalled(details, "topic");
				if (!topic) topic = sub->topic;
				(*sub->cbEvent)(wamp, subscriptionId, topic, details, list, dict);
				handled = 1;
			}
		}
	}
...
STATIC int wamp_HandleError(WAMP *wamp, JSON *json)
{
	...
	if (errorNumber == WAMP_INVOCATION) {					// An invocation error, need to pass back to the caller
		...
		client_registration_t *sub = idmap_GetValue(wamp->clientRegistrationsUnreq, requestId);
		if (sub) {
		...
		if (call) {
			if (call->cbResult)
				(*call->cbResult)(wamp, requestId, call->procedure, uri, details, list, dict);
...
API long long wamp_PublishIn(WAMP *wamp, const char *topic, JSON *opts, JSON *list, JSON *dict)
...
// The opts, list and dict become owned by the function.
{
	...
	JSON *json = wamp_NewMessage(WAMP_PUBLISH, requestId);

	json_ArrayAdd(json, opts ? opts : json_NewObject());
...
// NB. cbUnsubscribed will only be called if we DO NOT return a 0.
{
	...
	client_subscription_t *sub = idmap_GetValue(wamp->clientSubscriptionsSub, subscriptionId);
	if (!sub)
		sub = idmap_GetValue(wamp->clientSubscriptionsReq, subscriptionId);
...
API int wamp_Call(WAMP *wamp, const char *procedure, JSON *options, JSON *list, JSON *dict, WAMPCB_Result cbResult)
...
API long long wamp_Register(WAMP *wamp, int policy, int match, int priority, const char *procedure, WAMPCB_Registered cbRegistered, WAMPCB_Invoke cbInvoke)
{
	...
	JSON *options = json_NewObject();
	if (policy)
		json_ObjectAddStringz(options, "policy", wamp_PolicyName(policy));
	if (match)
		json_ObjectAddStringz(options, "match", wamp_MatchName(match));
	if (priority)
		json_ObjectAddInteger(options, "priority", priority);
	json_ArrayAdd(json, options);
	json_ArrayAddStringz(json, procedure);
...
// Returns 0 if we were not registered on this registration ID or are already unregistering
{
	...
	client_registration_t *reg = idmap_GetValue(wamp->clientRegistrationsReg, registrationId);
	if (!reg)
		reg = idmap_GetValue(wamp->clientRegistrationsReq, registrationId);
...
API void wamp_Hello(WAMP *wamp, const char *realm, JSON *caller, JSON *callee, JSON *publisher, JSON *subscriber, WAMPCB_Welcome cbWelcome)
...
{
	if (!wamp || !realm) return;

	JSON *hello = wamp_NewMessage(WAMP_HELLO, 0);
	json_ArrayAddStringz(hello, realm);

	wamp->cbWelcome = cbWelcome;

	JSON *roles = json_NewObject();
	if (caller)
		json_ObjectAdd(roles, "caller", caller);
	if (callee)
		json_ObjectAdd(roles, "callee", callee);
	if (publisher)
		json_ObjectAdd(roles, "publisher", publisher);
	if (subscriber)
		json_ObjectAdd(roles, "subscriber", subscriber);

	JSON *opts = json_NewObject();
	json_ObjectAdd(opts, "roles", roles);

	json_ArrayAdd(hello, opts);
...
API int wamp_Result(WAMP *wamp, long long requestId, JSON *options, JSON *list, JSON *dict)
{
	if (wamp) {
		if (!options) options = json_NewObject();
...
//Log("WAMP IN (%s):\n%s", wamp_Name(wamp), json_RenderPretty(json));
