...
STATIC char *SkipSpace(char *szText)
{
	if (szText)
		while (*szText == ' ' || *szText == '\t')
			szText++;
...
rogxml _ns_default[] = {
	{ROGXML_T_NS,NULL,NULL,"xsi","http://www.w3.org/2001/XMLSchema",NULL,NULL,&_ns_default[1],NULL},
	{ROGXML_T_NS,NULL,NULL,"xsl","http://www.w3.org/1999/XSL/Transform",NULL,NULL,&_ns_default[2],NULL},
	{ROGXML_T_NS,NULL,NULL,"xml","http://www.w3.org/XML/1998/namespace",NULL,NULL,NULL,NULL},
};
...
STATIC void rogxml_SetNamespace(rogxml *rx, rogxml *ns)
{
	if (rx) rx->nsName = ns;
}
...
STATIC void rogxml_ShowText(const char *szText)
{
	...
	if (!isValidUTF8(szText)) {						// 30667 The text is not valid UTF-8 so we'll make sure it is
		char *szCopy = malloc(strlen(szText)*2+1);	// Maximum we'll need
		char *dest=szCopy;
		char c;
		const char *chp;
	...
	if (_rogxml_fpout) {
		...
		size_t nNewLimit = (nNewLen + CHUNK - 1) & ~(CHUNK-1);
...
const char *szReadStringError = NULL;
...
STATIC void rogxlist_AddList(rogxlist *rxl, rogxlist *rxlAdd)
{
	rogxitem *it;
	if (!rxlAdd) return;

	for (it=rxlAdd->iFirst;it;it=it->iNext) {
		rogxlist_AddElement(rxl, it->rxElement);
	}
}
...
int rogxpath_FindNamespace(const char *szName)
...
void rogxpath_AddNamespace(const char *szName, const char *szURI)
...
// rogxpath_AddNamespace("blank",NULL) will mean that //blank:fred will only find <fred>
{
	...
	_rogxpath_Namespace=RENEW(_rogxpath_Namespace, char*, nCount+1);
	_rogxpath_Namespace[nCount]=strdup(szName);
	_rogxpath_Namespace[nCount+1]=NULL;

	_rogxpath_URI=RENEW(_rogxpath_URI, char*, nCount+1);
...
void rogxpath_ForgetNamespaces()
{
...
int MatchingElement(rogxml *rx, const char *szTest)
...
STATIC int SplitStep(char *szPath, char **pszAxis, char **pszTest, char **pszPredicates)
