...
API int s3_ErrorNo(S3 *s)
{
	...
	if (err == SQLITE_DONE) err = SQLITE_OK;				// Done means it wasn't a query, but a statement
...
// %S is the same as %s but doesn't enclose the result in '
...
// * Build the string using the array, freeing the copies as we go
{
	...
	strings=malloc(sizeof(char*)*percents);			// Enough for a pointer to each string
	...
	while (chp) {
		...
		if (chp) {
			...
			char *string = NULL;
			switch(format) {
			case 's':								// It's a string, so we need to quotify it
			case 'S':	{
							...
							if (s) {
								...
								string=malloc(strlen(s)+count+2+1);		// Len + space for extra 's + '' + \0
								char *output=string;
								if (format == 's') {
									*output++='\'';
								}
								...
								if (format == 's') strcat(output, "'");
							} else {
								string = strdup("NULL");
							}
						}
						break;
			case 'd':	string=malloc(30); snprintf(string, 30, "%ld", va_arg(ap, long)); break;
			case 'D':	string=malloc(30); snprintf(string, 30, "%Ld", va_arg(ap, long long)); break;
	...
	if (szFile) {
		...
		if (err) {
			if (must) {
				...
				if (buf) free(buf);
				buf = malloc(strlen(szFile)+9);

				if (szFile) {
					snprintf(buf, strlen(szFile)+9, "open(\"%s\")", szFile);
				} else {
					buf=strdup("open(NULL)");
				}
...
{
{ FILE *fp = fopen("/tmp/sqlite.log", "a"); fprintf(fp, "s3it_InternalFetchRow(%p, %d) stmt=%p, cached=%d\n", it, caching, it->stmt, it->isCached);fclose(fp);}
	const char **row = NULL;
	static const char **_savedRow = NULL;
...
{ FILE *fp = fopen("/tmp/sqlite.log", "a"); fprintf(fp, "%d=sqlite3_step(%p)\n", rc, pStmt); fclose(fp); }
...
//{FILE *fp = fopen("/tmp/sqlite.log", "a"); fprintf(fp, "%d=sqlite3_prepare_v2(%p, \"%s\", %d, %p, %p)\n", s3->lastErr, s3->sql3, qry, -1, &it->stmt, NULL); if (s3->lastErr) fprintf(fp, "ERR: %s\n", sqlite3_errmsg(s3->sql3)); fclose(fp);}
//		if (s3->lastErr == SQLITE_DONE) {					// It's not a SELECT etc.
//			s3->lastErr = sqlite3_step(it->stmt);
//{FILE *fp = fopen("/tmp/sqlite.log", "a"); fprintf(fp, "%d=sqlite3_step(%p)\n", s3->lastErr, &it->stmt); if (s3->lastErr) fprintf(fp, "ERR: %s\n", sqlite3_errmsg(s3->sql3)); fclose(fp);}
//		}
...
STATIC const char **addString(const char **argv, const char *string)
{
	...
	if (argv) {
		...
		argv = realloc(argv, (count+2)*sizeof(const char **));
	} else {
