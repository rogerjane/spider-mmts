...
// is non-NULL.
{
	...
	if (nMaxLen <= 0 || nMaxLen >= strlen(szAppend))	// Take the easy way out if possible
...
//printf("hprintf(\"%s\", \"%s\")\n", szOrig, szFmt);
	...
	if (nSent > (int)sizeof(buf)) {				// The LINUX runtime returns the number of bytes actually needed
		...
		if (szOrig) {
			...
			szResult = realloc((char*)szOrig, nLen+nSent+1);	// Reallocate to a new, bigger buffer
			if (!szResult) return NULL;					// We needed too much space...!
			aim=szResult+nLen;							// Where we want to write the new bit
		} else {
			aim=szResult=malloc(nSent+1);				// Give us a buffer just big enough
		}
		...
		if (szOrig) {
			...
			szResult=realloc((char*)szOrig, nLen+nSent+1);
...
//			cc*		String on the heap
{
	...
	while (!done) {
		if (fgets(buf, sizeof(buf), fp)) {
			...
			if (result) {
				result=realloc(result,sofar+len+1);
