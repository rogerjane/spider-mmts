...
STATIC const char *md5buf(void *ptr, int nLen)
{
	md5_state_t	state;
	static md5_byte_t digest[16];

	md5_init(&state);
	md5_append(&state, (const md5_byte_t *)ptr, nLen);
	md5_finish(&state, digest);

	return (const char *)digest;
}

guid *guid_FromSeedX(const char *szText, int nLen, unsigned int nType)
...
guid *guid_FromSeed(const char *szText, int nLen)
{
...
//!     a critical section.
guid *guid_Gen()
// Octet 8 top two bits must be 1, 0
{
	static guid g;
...
guid *guid_FromText(const char *szText)
{
...
int guid_Compare(const guid *a, const guid *b)
{
