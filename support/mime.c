...
STATIC const char *add_GetBuffer()		{ add_Flush(); return add_buff; }
...
//printf("In [%d]: %02x-%02x-%02x-%02x, Out [%d]: %02x-%02x-%02x\n", nLen, s[0], s[1], s[2], s[3], (i?(i-1):0), d[0], d[1], d[2]);
	return i ? (i-1) : 0;
}
...
// Returns NULL iff there is no memory left.
{
	...
	while (len>0) {
		int nDone=Base64E((len>=3)?3:len,szText, szDest);
		szText+=3;
		szDest+=4;
		len-=3;
		if (nWrap) {
...
// If pnLen is NULL then the result will be '\0' terminated as otherwise there is no way of knowing the length f'sure.
{
	...
	while ((c=*szText++) && (c != '=')) {
		...
		if ((c & 128) || (_base64D[c] & 128)) continue;	// Skip non-B64 characters
		place[i++]=c;
		if (i==4) {
			char buf[4];
...
int mime_GetLastError() { return mime_lasterror; }
const char *mime_GetLastErrorStr() { return mime_lasterrorstr; }
...
STATIC void TrimRight(char *szText)
{
	char *chp=szText+strlen(szText)-1;

	while (chp >= szText && isspace(chp))
		chp--;
...
// Unquotes a bit of text if it is quoted.
// This is done in situ so will bugger up the original and return a different pointer if the original string is quoted.
// Reduces any occurrances of '\"' in the string to just '"'
// Returns	Original string (if it wasn't quoted)
//			Pointer into modified string (if it was quoted)
{
	int nLen=strlen(szText);
	char *chp;

	if (nLen < 2 || *szText != '"' || szText[nLen-1] != '"') return szText;

	szText[nLen-1]='\0';
	szText++;

	chp=szText;
	while (chp=strchr(chp, '\\')) {				// Need to collapse \" pairs
		if (chp[1] == '"') {						// Got one
			memmove(chp, chp+1, strlen(chp));		// Copy remainder over the '\'
		}
		chp++;
	}
...
// Note that the vector (*ppv) WILL move during this call
{
	...
	pv=RENEW(pv, void*, n+2);	// Make the vector longer (existing entries + new + NULL)
...
STATIC void mimenv_SetName(MIMENV *nv, const char *name)
{
...
STATIC const char *mimenvp_RenderHeap(const char *heap, MIMENVP *nvp)
{
	...
	while (*pnv) {				// TODO: See if we really need this to be quoted for NPfIT
		if (!stricmp((*pnv)->name, "xxxboundary")) {						// Quote boundary
...
STATIC void mimenvp_SetName(MIMENVP *nvp, const char *name)		{ mimenv_SetName(nvp->nv, name); }
...
const char *mime_GetHeaderList(MIME *m)
{
...
// New with text held in a file
{
	MIMETEXT *t=mimetext_NewText(-1, szText);
	if (t) {
		t->bIsFile=1;
	}
...
const char *mimetext_GetContent(MIMETEXT *t)	{ return t ? t->szContent : NULL; }
int mimetext_GetLength(MIMETEXT *t)				{ return t ? t->nLen : 0; }

char mimetext_IsFile(MIMETEXT *t)				{ return t ? t->bIsFile : 0; }

////////// Things to do with content types

const char *mime_GetContentType(MIME *m)	{return mime_GetHeader(m, "Content-Type");}
...
void mime_ForceMultipart(MIME *m)
{
	...
	if (!mime_IsMultipart(m)) {						// Needs to be multipart so need a new part with old content
		...
		if (strnicmp(szOldType, "multipart/", 10))		// Make sure it has a multi-part type
...
#if 0
static const void *pSpareData;
static int nSpareData = 0;
...
static const char *mime_GetData(int *pnLen)
// Gets some data from the input
{
	if (nSpareData) {				// We have some returned
		*pnLen = nSpareData;
		nSpareData = 0;
		return pSpareData;
	}
	...
	if (mimefp) {
		...
		int got=fread(buf, 1, sizeof(buf), mimefp);
		if (got) {
			*pnLen=got;
			return buf;
		} else {
		...
		const char *szResult = (*mimereadfn)(pnLen);
		return szResult;
	}
...
//{FILE *fp=fopen("/tmp/chunking","a");fprintf(fp, "About to chunk...\n");fclose(fp);}
	if (szLine = mime_GetLine()) {
		...
		char *szName;			// Temporary place for chunk name
		int i;

//{FILE *fp=fopen("/tmp/chunking","a");fprintf(fp, "Chunk line is: '%s'\n", szLine);fclose(fp);}
		got = sscanf(szLine, "%x %s", &nLen, szName);
...
// Use NULL for szBoundary to ignore that.
{
	...
	char bIsBoundary = 0;							// 1 if non-terminating boundary
	...
	// bIsBoundary == 1		We've seen the boundary but it wasn't a final one
...
STATIC IsBoundary(const char *szText, const char *szBoundary, char *pbFinal)
// Checks the line passed to see if it matches the boundary, if it does then sets
// *pbFinal to say whether it was a final one.
{
	int nLen=strlen(szBoundary);

	if (strncmp(szText, "--", 2)) return 0;					// Doesn't start with "--"
	szText+=2;												// Skip the "--" for the rest of the tests

	if (strncmp(szText, szBoundary, nLen)) return 0;		// Doesn't match boundary on first part

	if (!szText[nLen]) {									// Exact match
		*pbFinal=0;
		return 1;
	}

	if (!strcmp(szText+nLen, "--")) {						// Terminating boundary
		*pbFinal=1;
		return 1;
	} else {												// Boundary followed by something else
		return 0;
	}
...
// Gets a token from a header line
// The contents of pszBuf will get mashed up through repeated calls to this function.
// Returns	Pointer to a token as a string (may not point into *pszBuf)
//			NULL	Nothing left on the line
{
	static char charbuf[2]="?";
	static char cSavedChar = '\0';
	char *buf;
	...
	switch (*buf) {
		...
		default:
			chp=buf;
			while (*chp && !isspace(*chp) && !strchr("\"()<>,;:\\[]", *chp))
				chp++;
			if (!*chp) {					// Very easy - end of the line
				*pszBuf=chp;
			} else if (isspace(*chp)) {		// Easy - followed by white space
				*chp='\0';
				*pszBuf=chp+1;
			} else {						// Trickier - need to save char for next time
				cSavedChar=*chp;			// Save until next time
...
// Gets a token from a header line, skipping comments.
// Actually, a quoted '(' on it's own will mess this up...
{
	const char *szToken = oldGetHeaderTokenRaw(pszBuf);
	if (!strcmp(szToken, "(")) return szToken;

	for (;;) {
		szToken = oldGetHeaderTokenRaw(pszBuf);
		if (!szToken) return NULL;					// Unterminated comment, return EOL
		if (!strcmp(szToken, ")")) {				// End of comment
			return oldGetHeaderToken(pszBuf);			// Recursively return next real token
		}
	}
}
...
//			for (;;) {							// I can use 'break' instead of goto...
				...
				if ((stricmp(szName, "Content-Type") &&	// Special case - always parse content-type so we have boundary
						!mime_bParseHeaderValues) ||	// Don't attempt to parse header values - just use rest of line
					(!stricmp(szName, "User-Agent") ||	// Also 'User-Agent: ' is never variable parsed
						!stricmp(szName, "Cache-Control") ||	// ... or 'Cache-Control: '
						!stricmp(szName, "Keep-Alive") ||		// ... or 'Keep-Alive: '
						!stricmp(szName, "Set-Cookie") ||		// ... or 'Set-Cookie: '
						!stricmp(szName, "Cookie") ||			// ... or 'Cookie: '
						!stricmp(szName, "Range") ||			// ... or 'Range: '
						!stricmp(szName, "Referer") ||			// ... or 'Referer: '
						!stricmp(szName, "Prefer") ||			// ... or 'Prefer: '
						!strnicmp(szName, "X-", 2) ||			// ... or anything starting 'X-'
						!stricmp(szName, "Accept"))) {			// ... or 'Accept: '
...
//{const char *szH=mime_RenderHeap(mime);printf("Header...\n%s", szH);szDelete(szH);}
	if (bWantBody) {
		...
		bMultipart = szType && !strnicmp(szType, "multipart/", 10);
...
//{FILE *fp=fopen("/tmp/mimeread","a");fprintf(fp, "Fetching body - multi=%d, boundary = '%s', Outer = '%s'\n", bMultipart, szBoundary, szOuterBoundary);fclose(fp);}
		if (bMultipart) {							// Go and get multiple parts
			...
			if (szTransferEncoding && !strcmp(szTransferEncoding, "chunked")) {		// Deal with chunks
				...
				while (szBodyChunk = mime_ReadBodyChunk(&nChunkLen, &szChunkName)) {		// Read a chunk
