...
// Four 'classes' to map strings to random data, strings, pointers or integers.
...
// NOT NULL SAFE (but only caller, smap_Add(), is)
{
	...
	for (i=0;i<sm->nArena;i++) {
		...
		if (szKey && szKey != SM_DELETED) {
			int nHash=(int)smap_Hash(szKey, sm->bIgnoreCase) & nMask;
			while (pKey[nHash]) {
				nHash++;
...
// This is the existing slot if it's there already or a blank one
{
	...
	int nHash=(int)smap_Hash(szKey, sm->bIgnoreCase) & sm->nMask;
...
// Creates a buffer on the heap, extending it by 'delta' byte increments
{
	...
	while (len--) {
		...
		if (*str) {
			...
			if (sublen+slen >= delta) {				// We're overflowing
				sublen=0;
				szResult = (char *)realloc(szResult, resultLen+delta);
			}
	...
	if (sublen == delta) szResult = (char *)realloc(szResult, resultLen+1);
	szResult[resultLen]='\0';

	return szResult;
}

// smap_Decode() isn't really API but I've put that to stop the compiler moaning.
// It should be called in smap_ReadFileEntry() but it's not - that should be corrected if it's ever needed

API const char *smap_Decode(char *szCoded)
...
// Returns -999 if sm is NULL (invalid)
{
	...
	nHash=(int)smap_Hash(szKey, sm->bIgnoreCase) & sm->nMask;
	int (*cmp)(const char *a, const char *b) = sm->bIgnoreCase ? strcasecmp : strcmp;
...
API long simap_GetValueAtIndex(SIMAP *sm, int index)					{ return (long)smap_GetValueAtIndex((SMAP*) sm, index); }
...
API int simap_Add(SIMAP *sm, const char *szKey, long nValue)
{
	return smap_Add((SMAP*)sm, szKey, (const char *)nValue, sizeof(long));
}

API long simap_GetValue(SIMAP *sm, const char *szKey, long nDefault)
// Fetches an int from the map, returning nDefault if not found
{
	int nIndex=smap_GetValueIndex((SMAP*)sm, szKey);
	return nIndex ? (long)smap_GetValueAt((SMAP*)sm, nIndex, NULL) : nDefault;
}

API const char *simap_GetKey(SIMAP *sm, long nValue)
{
...
API int sset_Add(SSET *set, const char *string)						{ return simap_Add((SIMAP*)set, string, 1L); }
...
API int sset_Contains(SSET *set, const char *string)				{ return simap_GetValue((SIMAP*)set, string, 0); }
