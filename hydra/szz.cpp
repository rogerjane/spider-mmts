...
API void szz_Dump(const char *szz, FILE *fp)
...
API const char *szz_Append(const char *szzA, const char *szB)
...
API int szz_Find(const char *szzA, const char *szB)
...
API int szz_CaseFind(const char *szzA, const char *szB)
// Returns the index of szB within szzA or 0 if szB isn't there.
// This version ignores the case of the string.
{
	if (szzA) {						// Don't bother if NULL and makes the whole thing NULL safe to boot
		...
		for (chp=szzA;*chp;chp=szz_Next(chp)) {
			nIndex++;
			if (!strcasecmp((char*)chp, (char*)szB)) {
				return nIndex;
			}
		}
	}
...
API const char *szz_UniqueAdd(const char *szzA, const char *szB)
...
API const char *szz_UniqueCatDelete(const char *szzA, const char *szzB)
...
API int szz_Size(const char *szz)
...
API const char *szz_Copy(const char *szz)
// Creates a copy of an szz
{
	...
	szzResult = (const char *)malloc(nLen);
	memcpy((char*)szzResult, szz, nLen);

	return szzResult;
}

API const char *szz_Join(const char *szz, const char *szConjunction)
...
//printf("Allocated %d+(%d-1)*%d = %d\n", nSize, nCount, nConLen, nSize+(nCount-1)*nConLen);
	szResult = (char *)malloc(nSize+(nCount-1)*nConLen);	// nSize included the terminating '\0';
